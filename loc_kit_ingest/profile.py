...
_PO_FIELDS = frozenset({
    "sheet", "component", "kind", "source_lang", "header_row",
    "first_data_row", "languages", "key", "comments", "references",
    "grammar",
})
_TBX_FIELDS = frozenset({
    "sheet", "component", "kind", "source_lang", "header_row",
    "languages", "grammar", "key_language", "initial_target_languages",
})
...
_REGION_FIELDS = frozenset({
    "section_row", "first_term_row", "last_description_row",
})
...
def _err(code: str, message: str, *, component: str = "", sheet: str = "", row: int = 0) -> ProfileError:
    return ProfileError(
...
def _check_unknown(obj: dict[str, Any], allowed: frozenset[str], *, label: str) -> None:
    ...
    if extras:
        raise _err("profile.unknown_field", f"unknown field(s) {extras} in {label}")


def _require(obj: dict[str, Any], field: str, *, label: str) -> Any:
    if field not in obj:
        raise _err("profile.missing", f"missing required field '{field}' in {label}")
    return obj[field]


def _require_int(obj: dict[str, Any], field: str, *, label: str, min_val: int = 1) -> int:
    val = _require(obj, field, label=label)
    if not isinstance(val, int) or isinstance(val, bool) or val < min_val:
        raise _err(
            "profile.invalid_index",
...
def _parse_language(obj: dict[str, Any], *, component: str) -> LanguageColumn:
    ...
    if not isinstance(code, str) or not _CODE_RE.fullmatch(code):
        raise _err("profile.invalid_code", f"invalid language code {code!r}")
    if not isinstance(xml_lang, str) or not _XML_LANG_RE.fullmatch(xml_lang):
        raise _err("profile.invalid_xml_lang", f"invalid xml_lang {xml_lang!r}")
    if not isinstance(header, str):
        raise _err("profile.invalid_header", f"invalid header {header!r}")
...
def _parse_metadata_column(obj: dict[str, Any], *, label: str) -> MetadataColumn:
    ...
    for field_val, field_name in [(name, "name"), (header, "header")]:
        if not isinstance(field_val, str):
            raise _err("profile.invalid_value", f"invalid {field_name} {field_val!r}")
...
def _parse_metadata_list(
    ...
    if not isinstance(items, list):
        raise _err("profile.invalid_value", f"'{field}' must be a list")
    return tuple(
        _parse_metadata_column(item, label=f"{field} entry")
        for item in items
    )
...
def _parse_skip_rows(obj: dict[str, Any], *, label: str) -> tuple[int, ...]:
    ...
    if not isinstance(val, list):
        raise _err("profile.invalid_value", f"'skip_rows' in {label} must be a list")
    result: list[int] = []
    for r in val:
...
def _parse_keyed_grammar(obj: dict[str, Any]) -> KeyedGrammar:
    ...
    if not isinstance(allow_blank, bool):
        raise _err("profile.invalid_value", "'allow_blank_rows' must be a boolean")
...
def _parse_pairs_grammar(obj: dict[str, Any], *, header_row_1based: int) -> PairsGrammar:
    ...
    if not isinstance(regions_raw, list) or not regions_raw:
        raise _err(
            "profile.missing_regions",
            "pairs grammar must have at least one region",
        )
    regions = tuple(_parse_region(r, header_row_1based=header_row_1based) for r in regions_raw)
...
def _parse_component(
    ...
    *,
    ...
    if not isinstance(obj, dict):
        raise _err("profile.invalid_value", "component must be an object")

    kind = obj.get("kind")
    if kind not in ("po", "tbx"):
        raise _err("profile.invalid_kind", f"unknown kind {kind!r}; must be 'po' or 'tbx'")
    ...
    if not isinstance(langs_raw, list) or not langs_raw:
        raise _err("profile.invalid_languages", "languages must be a non-empty list")
    languages = tuple(
        _parse_language(l, component=component) for l in langs_raw
    )
    ...
    if not isinstance(grammar_raw, dict):
        raise _err("profile.invalid_value", "grammar must be an object")
    grammar_type = grammar_raw.get("type")
    if kind == "po":
        if grammar_type != "keyed":
            raise _err(
                "profile.grammar_mismatch",
        ...
        grammar = _parse_pairs_grammar(
            grammar_raw, header_row_1based=header_row
        )
    ...
    if kind == "po":
        ...
        if not isinstance(key_raw, dict):
            raise _err("profile.invalid_value", "key must be an object")
...
def load_profile(path: Path) -> Profile:
    if not path.is_file():
        raise _err("profile.not_found", f"profile file not found: {path}")

    try:
        raw = path.read_text(encoding="utf-8")
    except OSError as exc:
        raise _err("profile.read_error", f"cannot read profile: {exc}") from exc

    try:
        data = json.loads(raw)
    except json.JSONDecodeError as exc:
        raise _err("profile.malformed_json", f"malformed JSON in profile: {exc}") from exc

    if not isinstance(data, dict):
        raise _err("profile.not_object", "profile root must be a JSON object")
