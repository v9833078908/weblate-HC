...
``docs/llm-first/``, ``docs/product/``, ``docs/operations/``, ``docs/guides/``
    ...
    ``llm-first`` — видение, роадмап и пайплайн LLM-first TMS, ``product`` —
    функции самого форка, ``operations`` — работа с живыми инстансами и играми,
    ``guides`` — вечнозелёные контракты и инструкции. Правило раскладки —
