...
``docs/product/``
    Единственное место для новых документов форка (в основном на русском) по
    игровой локализации. Все планы, дизайны, замеры, исследования, ревью,
    аудиты, отчёты, встречи, архив, видение и вечнозелёные руководства
    размещаются в ``docs/product/<жанр>/``. Новые документы не создаются в
    ``docs/llm-first/``, ``docs/operations/`` или ``docs/guides/``: это
    прежние расположения уже существующих файлов. Правило раскладки —
