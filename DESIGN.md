...
Оболочка: тёмная шапка в обычном потоке (не `fixed` и не `sticky`: в
`main.css` залипают только ячейки действий таблицы, `.sticky-header` и
`.sticky-bottom`) высотой ≈50px — `--bs-navbar-padding-y: 0` плюс ссылки с
`padding-y: 13px`; в токенах она нормализована до 48px. Дальше —
`container-fluid` во всю ширину. Полноширинная раскладка сохраняется:
таблицы строк и редактор перевода упираются в ширину экрана, поэтому
центрированный контейнер фиксированной ширины при переносе не вводится.
Залипание шапки — отдельное продуктовое решение, а не свойство текущей
темы; вводить его при переносе только явным изменением.
