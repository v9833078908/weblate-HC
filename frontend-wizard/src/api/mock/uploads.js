...
function baseAnalysis(scenario, id) {
  ...
  if (scenario === "utf16") {
    ...
    a.note =
      "Расширение файла — .txt, но по содержимому это TSV в UTF-16LE с BOM. Прочитали корректно.";
  }
  if (scenario === "multi_sheet") {
    a.format = {
      ...
      sheet: null,
      sheets: [
        { name: "Strings", rows: 3120 },
        { name: "Dialogues", rows: 744 },
        { name: "Meta", rows: 12 },
      ],
    };
    ...
    a.columns = [
      {
        ...
        role: "key",
        fill: 100,
        note: "ключи совпадают с английским текстом",
      },
      {
        header: "Russian",
        role: "language",
  ...
  if (scenario === "en_sparse") {
    a.columns = a.columns.map((c) =>
      c.header === "English" ? { ...c, fill: 41 } : c,
    );
  }
  return a;
}
  ...
  if (a.scenario === "en_sparse" && source === "en") {
    ...
    a.gate = {
      ...
      errors: [
        {
          row: "—",
          reason: "Колонка en заполнена на 41% — как оригинал не годится",
        },
      ],
      questions: [],
    };
    ...
    errors:
      dupPolicy === "stop"
        ? [
            {
              row: "141",
              reason: "Ожидается исправленный кит от разработчиков",
            },
          ]
        : [],
    questions: [
...
export function patchDraft(id, patch) {
  ...
  if (patch.explanation_language !== undefined)
    a.explanation_language = patch.explanation_language;
  if (patch.sheet !== undefined) {
    a.format.sheet = patch.sheet;
    a.stop = null;
  }
  if (patch.columns) a.columns = patch.columns;
  if (patch.languages) a.answers.languages = patch.languages;
  if (patch.duplicates_policy)
    a.answers.duplicates_policy = patch.duplicates_policy;
...
export function getExtraction(id) {
  ...
  if (idx >= STAGES.length)
    return {
      id,
      status: "done",
      stage: "Готовим предложения",
      stage_index: STAGES.length - 1,
      processed_rows: e.total_rows,
      total_rows: e.total_rows,
    };
  const processed = Math.min(
    e.total_rows,
    Math.round(((elapsed - 0.8) / (perStage * STAGES.length)) * e.total_rows),
  );
  return {
    id,
    status: "running",
