...
export function StageLanguages() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: mount-once language seeding from analysis; re-running would clobber user edits (existing eslint-disable).
  React.useEffect(() => {
    if (!answers.languages) {
      setAnswers({
        languages: initLanguages(analysis),
        source_language: answers.source_language || analysis.source_language,
      });
    }
  }, []); // eslint-disable-line
  ...
  const update = (key, patch) =>
    setLangs(langs.map((l) => (l.key === key ? { ...l, ...patch } : l)));
  const addPreset = (p) => {
    ...
    if (e) update(p.code, { include: true });
      ...
      setLangs([
        ...
        {
          key: p.code,
          group: p.group ? p.code : null,
          code: p.group ? null : p.code,
          name: p.name,
          source: "preset",
          include: true,
  ...
  const presetAvailable = PRESET.filter((p) => {
    const e = langs.find((l) => l.key === p.code);
    return !e?.include;
  });
    ...
    <section className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-body-sm text-muted-foreground">
          Один набор языков на весь проект. Существующие переводы не
          перезаписываются — прогон заполняет только пустые или изменившиеся
          строки.
        </p>
      </div>
      ...
      {analysis.source_options && (
        <SourceCard
          analysis={analysis}
          source={source}
          setSource={(c) => setAnswers({ source_language: c })}
      ...
      <div className="flex items-center gap-2 rounded-sm border border-border bg-muted px-3 py-2 text-body-sm text-foreground">
        ...
        Исходный язык: <span className="font-medium">{langName(source)}</span> —
        не изменится после настройки.
      </div>
      ...
      <div className="rounded border border-border bg-card p-4">
        ...
        {included.length === 0 ? (
          <p className="text-body-sm text-muted-foreground">
            Добавьте языки из пресета студии ниже.
          </p>
        ) : (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {l.source === "file" && !l.has_translation && (
                      <span className="rounded-sm bg-muted px-1.5 py-0.5 text-label-caps uppercase text-muted-foreground">
                        из файла
                      </span>
                    )}
                    {l.has_translation && (
                      <span className="rounded-sm bg-info-surface px-1.5 py-0.5 text-label-caps uppercase text-info-strong">
                        в файлах уже есть перевод
                      </span>
                    )}
                    {l.code && !l.has_translation && (
                  ...
                  <button
                    type="button"
                    onClick={() => update(l.key, { include: false })}
                ...
                {l.group && !l.code && (
                  <div className="mt-2">
                    <div className="mb-1 text-body-sm text-warning">
                      Выберите вариант — это разные локали:
                    </div>
                    <div className="flex flex-col gap-1">
                      {REGION[l.group].map((o) => (
                        <Radio
                          key={o.code}
                          name={`reg-${l.key}`}
                          checked={l.code === o.code}
                          onChange={() => update(l.key, { code: o.code })}
                        >
                          {o.label}
                        </Radio>
                      ))}
        ...
        <div className="rounded border border-border bg-card p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {presetAvailable.map((p) => (
              <Chip key={p.code} onClick={() => addPreset(p)}>
                + {p.name}
                {p.group && " *"}
              </Chip>
            ))}
          ...
          <p className="mt-2 text-body-sm text-muted-foreground">
            * потребуется выбрать региональный вариант.
          </p>
        </div>
      )}
      ...
      {unresolved.length === 0 && targets.some((l) => l.group) && (
        <div className="flex items-start gap-2 rounded-sm border border-success bg-success/10 p-3 text-body-sm text-foreground">
          ...
          <span>
            ...
            {targets
              .filter((l) => l.group)
              .map((l) => `${l.name.toLowerCase()} ${regionLabel(l.code)}`)
              .join(" · ")}
          </span>
        </div>
      )}
      ...
      <div className="rounded border border-border bg-muted p-4">
        ...
        {unresolved.length > 0 && (
          <div className="mt-2 flex items-center gap-1.5 text-body-sm text-warning">
            ...
            Выберите региональный вариант для:{" "}
            {unresolved.map((l) => l.name).join(", ")}
          </div>
        )}
      </div>
    </section>
...
function SourceCard({ analysis, source, setSource, setAnalysis }) {
  ...
  const addMissing = (code) => {
    setAnalysis({
      ...analysis,
      source_options: opts.map((o) =>
        o.code === code
          ? { ...o, have: o.need, complete: true, missing: [] }
          : o,
      ),
    });
    setSource(code);
  ...
  return (
    <div className="rounded border border-warning bg-card">
      <div className="flex items-center gap-2 border-b border-warning px-4 py-2.5">
        <AlertTriangle className="h-4 w-4 text-warning" />
        <h3 className="text-label-strong text-foreground">
          Исходный язык: комплект неполный
        </h3>
      </div>
      <div className="space-y-2 p-4">
          ...
          <div
            ...
            className={cn(
              ...
              o.complete
                ? "border-border"
                : "border-warning bg-warning-surface",
              source === o.code && o.complete && "border-primary bg-accent/40",
            )}
          >
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <div className="text-body-sm text-muted-foreground">
                  Доступно {o.have} из {o.need} исходных полей
                  {o.missing.length > 0 &&
                    ` · не хватает: ${o.missing.join(", ")}`}
                </div>
              </div>
              {o.complete ? (
                <Button
                  className="h-8 rounded-sm"
                  variant={source === o.code ? "default" : "secondary"}
                  onClick={() => setSource(o.code)}
                >
                  {source === o.code ? "Выбран" : "Выбрать"}
                </Button>
              ) : (
                <Button
                  className="h-8 rounded-sm"
                  variant="secondary"
                  onClick={() => addMissing(o.code)}
                >
                  Добавить недостающий файл
                </Button>
              )}
            </div>
          </div>
        ...
        <p className="text-body-sm text-muted-foreground">
          «Далее» заблокировано, пока не выбран полный исходный комплект.
          Русский текст не подставляется вместо отсутствующего английского
          автоматически.
        </p>
      </div>
    </div>
