...
import { FileSpreadsheet, Pencil, Store } from "lucide-react";
...
export function StageReview() {
  ...
  const saved = (answers.languages || []).filter(
    (l) => l.include && l.code && l.has_translation,
  );
  ...
  const units = isStore
    ? (analysis.fields?.length || 0) * targets.length
    : (analysis.rows?.importable || 0) * targets.length;
  const cost = isStore
    ? Math.max(1, Math.round(units * 0.35))
    : Math.round((analysis.rows?.importable || 0) * targets.length * 0.006);
    ...
    <section className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-body-sm text-muted-foreground">
          Проверьте настройки. Оценка ниже — только за перевод; проверка
          качества запускается отдельным прогоном.
        </p>
      </div>
      ...
      <div className="rounded border border-border bg-card p-4">
        ...
        <Row title="Исходный язык" onEdit={() => goto(2)}>
          {langName(source)}{" "}
          <span className="text-muted-foreground">· не изменится</span>
        </Row>
        <Row title="Целевые языки" onEdit={() => goto(2)}>
          {targets.length === 0 ? (
            <span className="text-warning">не выбраны</span>
          ) : (
            targets.map((c) => regionLabel(c)).join(" · ")
          )}
          {saved.length > 0 && (
            <div className="mt-0.5 text-muted-foreground">
              {saved.length} перевода сохраним (не перезапишем)
            </div>
          )}
        </Row>
        <Row title="Контекст" onEdit={() => goto(3)}>
          Обращение: {REG_UI[p.register_ui] || "—"} · Мат:{" "}
          {p.profanity === "keep"
            ? "сохраняем"
            : p.profanity === "soften"
              ? `смягчаем (${p.profanity_level === "none" ? "без мата" : "до лёгкой"})`
              : "—"}
        ...
        <Row title="Глоссарий" onEdit={() => goto(3)}>
          {g.skipped
            ? "пропущен"
            : selectedCount > 0
              ? `${selectedCount} терминов выбрано — добавится при запуске`
              : "без новых терминов"}
        </Row>
      </div>
      ...
      <div className="rounded border border-border bg-muted p-4">
        ...
        <div className="mt-1 text-body-sm text-muted-foreground">
          ≈ <Money value={cost} /> · ≈ {hMin}–{hMax} часа · письмо придёт по
          завершении
        </div>
      </div>
    </section>
