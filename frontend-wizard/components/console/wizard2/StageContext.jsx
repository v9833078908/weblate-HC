...
import { finalTargetCodes, useUni } from "@/components/console/wizard2/ctx";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";

const STORE_STAGES = [
  "Читаем тексты площадки",
  "Ищем названия и игровые понятия",
...
function Q({ n, title, hint, children }) {
  return (
    <div className="rounded border border-border p-4">
      ...
      {hint && (
        <p className="mb-2 ms-8 text-body-sm text-muted-foreground">{hint}</p>
      )}
...
export function StageContext() {
  ...
  const setP = (patch) =>
    setAnswers({ profile: { ...(answers.profile || {}), ...patch } });

  const g = answers.glossary || { skipped: false, selected: {} };
  const setG = (patch) =>
    setAnswers({
      glossary: {
        ...(answers.glossary || { skipped: false, selected: {} }),
        ...patch,
      },
    });
    ...
    <section className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-body-sm text-muted-foreground">
          Ответы задают тон перевода и служат опорой для проверки качества.
          Промпты формируются автоматически — ничего не выдумываем.
        </p>
      </div>
      ...
      <div className="space-y-4">
        <Q
          ...
          title={
            isStore
              ? "Тон текстов для стора"
              : "Обращение к игроку в интерфейсе"
          }
        >
          <Radio
            name="rui"
        ...
        <Q
          n={isStore ? 2 : 3}
          title="Мат и грубость сохраняем в силе источника?"
        >
          <Radio
            name="prof"
        ...
        <div className="p-4">
          {g.skipped ? (
            <p className="text-body-sm text-muted-foreground">
              Глоссарий пропущен. Можно добавить термины позже на странице
              «Глоссарий» — прогон запустится и без него.
            </p>
          ) : (
            <GlossaryWorkspace
              ...
              mode="wizard"
              targetLanguages={
                targets.length ? targets : ["en", "de", "fr", "ja"]
              }
              categories={{
                item: true,
                character: true,
              ...
              sourceLabel={
                isStore ? "тексты площадки (текущая версия)" : undefined
              }
              beforeTitle={
                isStore ? "Предложить термины из загруженных файлов" : undefined
              }
              beforeDesc={
                isStore
