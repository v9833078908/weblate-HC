...
export function ClarifyCard({
  ...
  const border =
    tone === "destructive" ? "border-destructive" : "border-warning";
  ...
  return (
    <section className={cn("rounded border bg-card", border)}>
      ...
      <div className="p-4">
        {title && (
          <div className="mb-3 text-body-md text-foreground">{title}</div>
        )}
        {children}
      </div>
    </section>
...
export function FieldCounter({ f }) {
  ...
  return (
    <span className={cn("tabular-nums", tone)}>
      {f.limit > 0
        ? `${f.chars.toLocaleString("ru-RU")} / ${f.limit.toLocaleString("ru-RU")}`
...
function StatusMark({ f }) {
  ...
  if (f.status === "over")
    return (
      <span className="inline-flex items-center gap-1 text-body-sm text-destructive">
  ...
  return (
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      <SheetContent className="w-full overflow-y-auto sm:max-w-[560px]">
        <SheetHeader>
          <SheetTitle>
            {isStore ? "Проверка полей" : "Анализ лок-кита"}
          </SheetTitle>
        </SheetHeader>
          ...
          {isStore ? (
            <>
              <div className="rounded-sm border border-border p-3 text-body-sm">
                ...
                <div className="mt-1 text-foreground">
                  {analysis.store} · лимиты:{" "}
                  {analysis.fields?.[0]?.limit_source}
                </div>
              </div>
                ...
                {(analysis.fields || []).map((f) => (
                  ...
                  >
                    ...
                    <div className="mt-1 flex items-center gap-2 text-body-sm text-muted-foreground">
                      ...
                      {f.markup && <span>· {f.markup}</span>}
                    </div>
                    <div className="mt-2 whitespace-pre-wrap rounded-sm bg-muted p-2 text-body-sm text-foreground">
                      {f.value || (
            ...
            <>
              <div className="rounded-sm border border-border p-3 text-body-sm">
                ...
                <div className="mt-1 text-foreground">
                  {analysis.format?.kind} · {analysis.format?.encoding}
                  {analysis.format?.sheet
                    ? ` · лист ${analysis.format.sheet}`
                    : ""}
                </div>
              </div>
              <div className="rounded-sm border border-border p-3">
                <div className="mb-1 text-label-caps uppercase text-muted-foreground">
                  Строки
                </div>
                <div className="text-body-sm text-foreground">
                  Всего {analysis.rows?.total?.toLocaleString("ru-RU")} · готовы{" "}
                  {analysis.rows?.importable?.toLocaleString("ru-RU")} ·
                  карантин {analysis.rows?.quarantine}
                </div>
                {analysis.rows?.by_reason &&
                  Object.keys(analysis.rows.by_reason).length > 0 && (
                    <ul className="mt-2 space-y-0.5 text-body-sm text-muted-foreground">
                      {Object.entries(analysis.rows.by_reason).map(([k, v]) => (
                        <li key={k}>
                          {REASON_RU[k] || k}: {v}
                        </li>
                      ))}
              ...
              <div className="rounded-sm border border-border p-3">
                ...
                <ul className="space-y-1 text-body-sm">
                  {(analysis.columns || []).map((c) => (
                    <li key={c.header} className="flex justify-between">
              ...
              <div className="rounded-sm border border-border p-3">
                ...
                <div className="flex flex-wrap gap-1.5 text-body-sm">
                  {(analysis.languages_found || []).map((l) => (
                    <span
                      key={l.code}
...
const REASON_RU = {
  empty_all: "пусто во всех языках",
  duplicate_exact: "точный дубликат",
  duplicate_conflict: "конфликт дубликатов",
...
export function ClarifyDispatcher({ c, analysis, patch, registry }) {
  ...
  switch (c.code) {
    case "unknown_files":
      return (
        <UnknownFilesCard c={c} onClarify={onClarify} resolved={c.resolved} />
      );
    case "ambiguous_content":
      return (
        <AmbiguousContentCard
          c={c}
          onClarify={onClarify}
          registry={registry}
          resolved={c.resolved}
    ...
    case "two_stores":
      return (
        <TwoStoresCard c={c} onClarify={onClarify} resolved={c.resolved} />
      );
    ...
    case "duplicate_field":
      return (
        <DuplicateFieldCard c={c} onClarify={onClarify} resolved={c.resolved} />
      );
...
function UnknownFilesCard({ c, onClarify, resolved }) {
  const setFile = (i, patch) => {
    const files = c.files.map((f, j) => (j === i ? { ...f, ...patch } : f));
    onClarify({ files });
  };
  return (
    <ClarifyCard
      resolved={resolved}
      title={`Не узнали ${c.files.length} имени файла. Укажите, какие это поля.`}
    ...
    >
      <div className="space-y-4">
        {c.files.map((f, i) => (
          <div key={f.name} className="rounded-sm border border-border p-3">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div>
                <Label className="text-body-sm">Название поля</Label>
                <Input
                  className="mt-1 h-9"
                  value={f.label}
                  onChange={(e) => setFile(i, { label: e.target.value })}
                  placeholder="Напр. Описание проекта"
                />
              </div>
              <div>
              ...
              {!f.no_limit && f.limit != null && (
                <div className="md:col-span-2">
                  <Label className="text-body-sm">
                    Источник лимита <span className="text-destructive">*</span>
                  </Label>
                  <Input
                    className="mt-1 h-9"
                    value={f.limit_source}
                    onChange={(e) =>
                      setFile(i, { limit_source: e.target.value })
                    }
                    placeholder="Откуда лимит (справочник, документация)"
                  />
                </div>
              )}
              <div className="md:col-span-2">
        ...
        <p className="text-body-sm text-muted-foreground">
          Неизвестный лимит не придумываем. Если указываете лимит — укажите
          источник.
        </p>
      </div>
    </ClarifyCard>
...
function AmbiguousContentCard({ c, onClarify, registry, resolved }) {
  ...
  return (
    <ClarifyCard
      resolved={resolved}
      title={`Что находится в файле ${c.file}?`}
    ...
    >
      <div className="space-y-2">
        {[
          ["table", "Таблица строк лок-кита"],
          ["store_field", "Одно поле страницы стора"],
          ["other", "Другое"],
        ].map(([v, l]) => (
          <Radio
            key={v}
            name={`amb-${c.id}`}
            checked={c.choice === v}
            onChange={() => onClarify({ choice: v, store: null, field: null })}
          >
            {l}
          </Radio>
        ))}
              ...
              <div className="mt-1 flex flex-wrap gap-2">
                {stores.map((s) => (
                  <Chip
                    key={s.id}
                    active={c.store === s.id}
                    onClick={() => onClarify({ store: s.id, field: null })}
                  >
                    {s.label}
                  </Chip>
                ))}
...
function TwoStoresCard({ c, onClarify, resolved }) {
  return (
    <ClarifyCard
      resolved={resolved}
      title="Нашли тексты для двух площадок"
      ...
      <ul className="mb-3 space-y-1.5">
        {c.stores.map((s) => (
          ...
          >
            ...
            <span className="text-muted-foreground">{s.field_count} поля</span>
          </li>
        ))}
      </ul>
      <p className="mb-3 text-body-sm text-foreground">
        Создать два отдельных набора: Steam и Google Play?
      </p>
      <div className="flex flex-wrap gap-2">
        <Button
          className="h-9 rounded-sm"
          onClick={() => onClarify({ choice: "split" })}
        >
          Создать два набора
        </Button>
        <Button
          variant="secondary"
          className="h-9 rounded-sm"
          onClick={() => onClarify({ choice: "manual" })}
        >
      ...
      <div className="space-y-2">
        {fields.map((f) => (
          <Radio
            key={f.id}
            name={`uik-${c.id}`}
            checked={c.choice === f.id}
            onChange={() => onClarify({ choice: f.id })}
          >
            {f.label}
          </Radio>
        ))}
        <Radio
          name={`uik-${c.id}`}
          checked={c.choice === "additional"}
          onChange={() => onClarify({ choice: "additional" })}
        >
          Дополнительное поле
        </Radio>
        <Radio
          name={`uik-${c.id}`}
          checked={c.choice === "skip"}
          onChange={() => onClarify({ choice: "skip" })}
        >
          Не импортировать
        </Radio>
        {c.choice === "additional" && (
          <div className="ms-6 mt-2 grid grid-cols-1 gap-3 rounded-sm border border-border p-3 md:grid-cols-2">
            <div>
              <Label className="text-body-sm">Русское название</Label>
              <Input
                className="mt-1 h-9"
                value={c.extra.label}
                onChange={(e) => setExtra({ label: e.target.value })}
              />
            </div>
            <div>
              <Label className="text-body-sm">Лимит</Label>
              <div className="mt-1 flex items-center gap-2">
                <Input
                  className="h-9 w-24"
                  type="number"
                  disabled={c.extra.no_limit}
                  value={c.extra.limit ?? ""}
                  onChange={(e) =>
                    setExtra({
                      limit: e.target.value ? Number(e.target.value) : null,
            ...
            {!c.extra.no_limit && c.extra.limit != null && (
              <div className="md:col-span-2">
                <Label className="text-body-sm">
                  Источник лимита <span className="text-destructive">*</span>
                </Label>
                <Input
                  className="mt-1 h-9"
                  value={c.extra.limit_source}
                  onChange={(e) => setExtra({ limit_source: e.target.value })}
                />
              </div>
            )}
            <div className="md:col-span-2">
              ...
              <div className="mt-1 flex gap-2">
                {MARKUP_OPTS.map(([v, l]) => (
                  <Chip
                    key={v}
                    active={c.extra.markup === v}
                    onClick={() => setExtra({ markup: v })}
                  >
                    {l}
                  </Chip>
                ))}
...
function DuplicateFieldCard({ c, onClarify, resolved }) {
  return (
    <ClarifyCard
      resolved={resolved}
      title={`Для поля «${c.field_label}» найдено два файла. Выберите один.`}
    ...
    >
        ...
        {c.candidates.map((f) => (
          ...
          >
            ...
            <div className="mt-1 text-body-sm text-muted-foreground">
              {f.chars} симв. · {f.markup} · изменён {f.modified}
            </div>
            <ul className="mt-2 space-y-0.5 rounded-sm bg-muted p-2 text-body-sm text-foreground">
              {f.preview.map((p, i) => (
                // biome-ignore lint/suspicious/noArrayIndexKey: static preview strings, no stable id
            ...
            <Button
              ...
              variant={c.choice === f.file ? "default" : "secondary"}
              onClick={() => onClarify({ choice: f.file })}
            >
      ...
      <p className="mt-2 text-body-sm text-muted-foreground">
        Файлы нельзя объединять автоматически.
      </p>
    </ClarifyCard>
  );
}
...
function OverLimitCard({ c, analysis, onFix, resolved }) {
  ...
  const replace = () =>
    onFix({
      field: { id: f.id, value: good.replace(/[\s:—-]+$/, "") },
      clarify: { id: c.id, resolved: true },
    });
  return (
    <ClarifyCard
      resolved={resolved}
      tone="destructive"
      title={`${f.label} превышает лимит`}
      summary={`Уточнения завершены: «${f.label}» в пределах лимита`}
    >
      <div className="text-body-md tabular-nums text-destructive">
        {f.chars} из {f.limit} символов
      </div>
      <div className="mt-1 text-body-sm text-muted-foreground">
        Лимит {analysis.store} · источник: {f.limit_source}
      </div>
      <div className="mt-3 whitespace-pre-wrap rounded-sm border border-border bg-muted p-3 text-body-sm text-foreground">
        {good}
        <mark className="rounded-sm bg-destructive/20 px-0.5 text-destructive line-through">
          {over}
        </mark>
      </div>
      <div className="mt-3 flex flex-wrap gap-2">
        <Button className="h-9 rounded-sm" onClick={replace}>
          Заменить файл
        </Button>
        <Button
          variant="ghost"
        ...
        >
          Посмотреть источник лимита
        </Button>
      </div>
      {showSrc && (
        <div className="mt-2 rounded-sm bg-info-surface p-2 text-body-sm text-info-strong">
          Лимит {f.limit} символов для поля «{f.label}» — {f.limit_source}.
        </div>
      )}
      <p className="mt-2 text-body-sm text-muted-foreground">
        Текст не обрезается автоматически. Остальные файлы и ответы сохраняются.
      </p>
    </ClarifyCard>
  );
}
...
function BrokenBBCodeCard({ c, analysis, onFix, resolved }) {
  ...
  const fixed =
    "Пиратские корабли — [b]морской рогалик[/b] о капитане, что бросает вызов проклятию Мёртвой оболочки. Собирайте команду и покоряйте штормовые воды.";
  return (
    <ClarifyCard
      resolved={resolved}
      tone="destructive"
      title={`В поле «${f.label}» не закрыт тег [${err.tag}]`}
      summary={`Уточнения завершены: разметка поля «${f.label}» исправлена`}
    >
      <div className="rounded-sm border border-border bg-muted p-3 text-body-sm text-foreground">
        {err.fragment.split(`[${err.tag}]`).map((part, i, arr) => (
          // biome-ignore lint/suspicious/noArrayIndexKey: parts of a string split are positional by nature
          <React.Fragment key={i}>
            {part}
            {i < arr.length - 1 && (
              <mark className="rounded-sm bg-destructive/20 px-0.5 font-mono text-destructive">
                [{err.tag}]
              </mark>
            )}
          </React.Fragment>
      ...
      <p className="mt-2 text-body-sm text-muted-foreground">
        Разметку не исправляем автоматически. Остальные файлы сохраняются.
      </p>
    </ClarifyCard>
  );
}
...
function EmptyFileCard({ c, analysis, onFix, resolved }) {
  ...
  return (
    <ClarifyCard
      resolved={resolved}
      title={`В файле ${f?.file || c.field} нет текста`}
      summary={`Уточнения завершены: поле «${label}» ${f ? "заполнено" : "удалено из набора"}`}
    >
      <p className="text-body-sm text-muted-foreground">
        Пустое исходное поле невозможно перевести.
        {c.required
          ? " Это поле обязательно для набора."
          : " Поле необязательно — набор может продолжить без него."}
      </p>
      <div className="mt-3 flex flex-wrap gap-2">
        {!c.required && (
          <Button
...
export function Chip({ active, onClick, children, disabled }) {
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "rounded-sm border px-3 py-1 text-body-sm transition-colors",
        active
...
export function InfoNote({ children }) {
  return (
    <div className="flex items-start gap-2 rounded-sm bg-info-surface p-3 text-body-sm text-info-strong">
      ...
      {fields.map((f) => (
        ...
        >
          <div className="min-w-0">
            ...
            <div className="truncate font-mono text-body-sm text-muted-foreground">
              {f.file}
            </div>
          </div>
