...
import { UniCtx, initLanguages, finalTargetCodes } from '@/components/console/wizard2/ctx'
...
const STAGE_NAMES = ['Файлы', 'Языки', 'Контекст и термины', 'Проверка и запуск']
...
export function UniversalWizard({ project, navigate }) {
  ...
  const [answers, setAnswersState] = React.useState({ source_language: null, languages: null, profile: null, glossary: { skipped: false, selected: {} } })
  const [uploading, setUploading] = React.useState(false)
  const [registry, setRegistry] = React.useState({})
  const [submitting, setSubmitting] = React.useState(false)

  const setAnswers = React.useCallback((patch) => setAnswersState((p) => ({ ...p, ...(typeof patch === 'function' ? patch(p) : patch) })), [])

  React.useEffect(() => { api.getStoresRegistry().then(setRegistry).catch(() => {}) }, [])

  // restore / deep-link
  React.useEffect(() => {
    const q = new URLSearchParams(window.location.search)
    const u = q.get('u'); const scenario = q.get('scenario'); const stg = Number(q.get('stage') || 0)
    if (u) {
      api.getUniversalUpload(u).then((a) => {
        setAnalysis(a)
        setAnswersState((p) => ({ ...p, languages: initLanguages(a), source_language: a.source_language }))
        if (stg) { setStep(stg); setMaxReached(stg) }
      }).catch(() => {})
    } else if (scenario) {
      const jump = stg || (typeof SCENARIOS[scenario]?.stage === 'number' ? SCENARIOS[scenario].stage : 1)
      startUpload(scenario, jump)
    }
  }, []) // eslint-disable-line

  const startUpload = React.useCallback((input, jumpTo) => {
    setUploading(input)
    api.createUniversalUpload(slug, input).then((a) => {
      setUploading(false)
      setAnalysis(a)
      setAnswersState({ languages: initLanguages(a), source_language: a.source_language, profile: null, glossary: { skipped: false, selected: {} } })
      const sp = new URLSearchParams(); sp.set('u', a.id)
      window.history.replaceState({}, '', `/projects/${slug}/localize?${sp.toString()}`)
      if (jumpTo && jumpTo > 1) { setStep(jumpTo); setMaxReached((m) => Math.max(m, jumpTo)) }
    }).catch(() => setUploading(false))
  }, [slug])

  const resetUpload = React.useCallback(() => {
    setAnalysis(null); setStep(1); setMaxReached(1)
  ...
  const goto = React.useCallback((n) => { setStep(n); setMaxReached((m) => Math.max(m, n)) }, [])
  ...
  const canProceed = React.useMemo(() => {
    ...
    if (step === 2) {
      ...
      const sourceOk = !analysis?.source_options || !!(analysis.source_options.find((o) => o.code === source)?.complete)
      return tgt.length > 0 && unresolved.length === 0 && sourceOk
    }
    if (step === 3) {
      const p = answers.profile || {}
      const dialogueOk = isStore ? true : !!p.register_dialogue
      return !!p.register_ui && !!p.profanity && dialogueOk && (!cjkNeeded || !!p.cjk_politeness)
    }
    return true
  ...
  const submit = async () => {
    ...
    const run = await api.createUniversalLocalization(slug, {
      kind: analysis.kind, store: analysis.store, upload_id: analysis.id,
      source_language: source, languages: targets, profile: answers.profile,
  ...
  const ctx = { slug, navigate, analysis, setAnalysis, answers, setAnswers, patch, registry, goto, step, uploading, startUpload, resetUpload }
      ...
      <AppShell project={project} active="overview" navigate={navigate}
        ...
        <div className="mb-6 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <Button variant="ghost" className="h-9 rounded-sm" onClick={() => navigate(`/projects/${slug}/localize-legacy`)}>Старый мастер</Button>
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-[240px_1fr]">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-8 flex items-center justify-between gap-3 border-t border-border pt-4">
              ...
              <div className="flex items-center gap-2">
                {step < 4 && <Button className="h-9 rounded-sm" disabled={!canProceed} onClick={() => goto(step + 1)}>Далее <ArrowRight className="h-4 w-4" /></Button>}
                {step === 4 && <Button className="h-9 rounded-sm" disabled={submitting || targets.length === 0} onClick={submit}><Rocket className="h-4 w-4" />{submitting ? 'Запуск…' : 'Сделать локализацию'}</Button>}
              </div>
            </div>
          </div>
        </div>
