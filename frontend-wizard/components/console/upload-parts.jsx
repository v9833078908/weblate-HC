...
export function DropZone({
  ...
  label = "Перетащите файл сюда или выберите на диске",
}) {
  ...
  if (parsing) {
    return (
      <div className="flex flex-col items-center justify-center rounded border border-border bg-muted p-10 text-center">
        ...
        {fileName && (
          <div className="mt-1 font-mono text-body-sm text-muted-foreground">
            {fileName}
          </div>
        )}
      </div>
  ...
  return (
    // biome-ignore lint/a11y/noStaticElementInteractions: drag-and-drop file target; drag handlers are pointer-only by nature, the paired click/browse control provides keyboard access
    <div
      onDragOver={(e) => {
        e.preventDefault();
        setDrag(true);
      ...
      onDrop={(e) => {
        e.preventDefault();
        setDrag(false);
        onPick(e.dataTransfer.files?.[0]?.name || "loc-kit.xlsx");
      }}
      className={cn(
        "flex flex-col items-center justify-center rounded border-2 border-dashed p-10 text-center transition-colors",
        drag ? "border-primary bg-muted" : "border-input bg-card",
      )}
    >
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        <Button
          variant="ghost"
          className="h-9 rounded-sm"
          onClick={() => onPick("pirate-ships-loc-kit.xlsx")}
        >
          Использовать пример
        </Button>
      </div>
      <input
        ...
        type="file"
        className="hidden"
        onChange={(e) => onPick(e.target.files?.[0]?.name || "loc-kit.xlsx")}
      />
    </div>
  );
}
...
export function LocKitPreview({ preview, showDiff }) {
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="rounded border border-border p-4">
          ...
          <div className="mt-1 text-body-md text-foreground">
            {preview.source_language} — первая заполненная колонка
          </div>
          <div className="mt-1 text-body-sm text-muted-foreground">
            Исходный язык нельзя изменить после настройки локализации.
          </div>
        </div>
        <div className="rounded border border-border p-4">
          <div className="text-label-caps uppercase text-muted-foreground">
          ...
          <div className="mt-1 text-metric-lg tabular-nums text-foreground">
            {preview.rows.toLocaleString("ru-RU")}
          </div>
        </div>
      </div>
      ...
      <div className="rounded border border-border p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {preview.languages.map((l) => (
            <span
              key={l.code}
              className="inline-flex items-center gap-1.5 rounded-sm border border-border bg-muted px-2 py-1 text-body-sm"
            >
              {l.name}{" "}
              ...
              <span className="text-muted-foreground">
                ·{" "}
                {l.prefilled > 0
                  ? `${l.prefilled.toLocaleString("ru-RU")} переводов`
                  : "пусто"}
              </span>
            </span>
          ))}
      ...
      {showDiff && preview.diff && (
        <div className="rounded border border-border bg-muted p-4">
          ...
          <div className="grid grid-cols-2 gap-y-1 text-body-sm md:grid-cols-4">
            <div>
              ...
              <span className="font-semibold text-foreground">
                {preview.diff.new_keys}
              </span>
            </div>
            <div>
              Изменённый источник{" "}
              <span className="font-semibold text-foreground">
                {preview.diff.changed_source}
              </span>
            </div>
            <div>
              Без изменений{" "}
              <span className="font-semibold text-foreground">
                {preview.diff.unchanged.toLocaleString("ru-RU")}
              </span>
            </div>
            <div>
              Удалённых{" "}
          ...
          <p className="mt-2 text-body-sm text-muted-foreground">
            Существующие переводы не перезаписываются — переводятся только новые
            ключи и строки с изменённым источником.
          </p>
        </div>
      )}
        ...
        <Collapsible className="rounded border border-border">
          <CollapsibleTrigger className="flex w-full items-center justify-between gap-2 p-4 text-start">
            <span className="flex items-center gap-2 text-body-md text-foreground">
              ...
              Строки без текста ({preview.quarantine.length}) — импортированы,
              но отложены
            </span>
            <ChevronDown
      ...
      <div className="rounded border border-border p-4">
        <div className="mb-2 text-label-caps uppercase text-muted-foreground">
          Сопоставление колонок (только чтение)
        </div>
        <div className="flex flex-wrap gap-2 text-body-sm">
          ...
          {preview.languages.map((l) => (
            <span
              key={l.code}
              className="rounded-sm bg-muted px-2 py-1 font-mono"
            >
              {l.code} → {l.code}
            </span>
          ))}
...
export const LANG_NAMES = {
  ...
  ja: "Японский",
  ko: "Корейский",
  zh_Hans: "Китайский (упр.)",
  zh_Hant: "Китайский (трад.)",
  id: "Индонезийский",
};
