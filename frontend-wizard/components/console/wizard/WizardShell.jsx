...
function buildLanguages(upload) {
  const inKit = upload.columns
    .filter((c) => c.role === "language")
    .map((c) => ({
      header: c.header,
      code: c.language,
      fill: c.fill,
      ambiguous: c.ambiguous || null,
      low_fill: !!c.low_fill,
      include: !c.low_fill && !c.ambiguous ? true : !c.ambiguous,
      resolved_as: c.ambiguous ? null : c.language,
      source: "kit",
    }));
  const kitCodes = new Set(inKit.map((l) => l.resolved_as || l.code));
  const preset = LANG_PRESET.filter((p) => !kitCodes.has(p.code)).map((p) => ({
    header: null,
    code: p.code,
    fill: null,
    ambiguous: null,
...
export function WizardShell({ project, navigate }) {
  ...
  const [s, setS] = React.useState({
    ...
    splitMode: "single",
    split: { rule: [], residue: "", ambiguity: {} },
    profile: {
      bdhc_title_id: null,
      store_url: "",
      questionnaire: null,
      register_ui: null,
      register_dialogue: null,
      profanity: null,
    ...
    glossary: {
      ...
      exceptionsApproved: {},
      exceptionsRejected: {},
      materials: { read_only: "", exact: "", forbidden: "" },
    },
  });
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: mount-once draft restore from ?u= (existing eslint-disable).
  React.useEffect(() => {
      ...
      api
        ...
        .then((a) => {
          ...
          set({
            ...
            languages: buildLanguages(a),
          });
          if (st) {
            setStep(Number(st));
            setMaxReached(Number(st));
          }
        })
        .catch(() => {});
    }
  ...
  const onParsed = (a) => {
    ...
    window.history.replaceState(
      {},
      "",
      `/projects/${slug}/localize?u=${a.id}&step=1`,
    );
  };
  ...
  const goto = (n) => {
    ...
    if (s.uploadId)
      window.history.replaceState(
        {},
        "",
        `/projects/${slug}/localize?u=${s.uploadId}&step=${n}`,
      );
  };
  ...
  const submit = async () => {
    ...
    const run = await api.createLocalization(slug, {
      ...
      explanation_language: s.explanation_language,
      languages: langs,
      columns: upload?.columns,
      duplicates_policy: s.duplicates_policy,
      split: s.splitMode === "split" ? s.split : undefined,
      profile: s.profile,
      glossary: { terms: s.glossary.selected },
    });
    setSubmitting(false);
  ...
  const canProceed = React.useMemo(() => {
    ...
    if (step === 3) {
      const langs = s.languages || [];
      const ambiguousResolved = langs
        .filter((l) => l.ambiguous)
        .every((l) => !!l.resolved_as || l.include === false);
      const idDecided =
        !upload?.columns.some((c) => c.header === "Id") ||
        s.idIsLanguage !== null;
      const hasTarget = langs.some(
        (l) => l.include && (l.resolved_as || l.code) !== s.source_language,
      );
      const rareDecided = langs
        .filter((l) => l.low_fill)
        .every(
          (l) =>
            l._decided === true || l.include === false || l.include === true,
        );
    ...
    if (step === 6) {
      ...
      return (
        s.profile.register_ui &&
        s.profile.register_dialogue &&
        s.profile.profanity &&
        (!cjkNeeded || s.profile.cjk_politeness)
      );
    }
  ...
  const skipped = {
    5: s.splitMode === "single" && step > 5,
    7: s.glossary.skipped,
  };
    ...
    <WizardCtx.Provider value={ctx}>
      ...
      >
        <div className="mb-6 flex items-center justify-between gap-3">
          ...
          <Button variant="ghost" className="h-9 rounded-sm" onClick={cancel}>
            Отмена
          </Button>
        </div>
              ...
              <div className="flex items-center gap-2">
                ...
                {step === 7 && (
                  <Button
                    ...
                    onClick={() => {
                      set((p) => ({
                        glossary: { ...p.glossary, skipped: true },
                      }));
                      goto(8);
                    }}
                  >
