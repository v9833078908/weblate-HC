...
export function EmailPreviewScreen({ kind = "completion", navigate }) {
  ...
  return (
    <AppShell
      navigate={navigate}
      breadcrumb={[{ label: "Проекты", path: "/" }, { label: "Письмо" }]}
    >
      <div className="mx-auto max-w-2xl">
        <div className="mb-4 flex gap-2">
          <Button
            variant={kind === "completion" ? "default" : "secondary"}
          ...
          <Button
            variant={kind === "failure" ? "default" : "secondary"}
            className="h-8 rounded-sm"
        ...
        <div className="overflow-hidden rounded border border-border">
          <div className="border-b border-border bg-muted px-4 py-3 text-body-sm">
            <div className="text-muted-foreground">
              От: <span className="text-foreground">{email.from}</span>
            </div>
            <div className="text-muted-foreground">
              Кому: <span className="text-foreground">{email.to}</span>
            </div>
            <div className="mt-1 text-heading-card text-foreground">
              {email.subject}
            </div>
          </div>
          <div className="space-y-4 bg-card p-6">
            ...
            <Button
              ...
              onClick={() => navigate(email.cta_url)}
            >
              {email.cta}
            </Button>
          </div>
