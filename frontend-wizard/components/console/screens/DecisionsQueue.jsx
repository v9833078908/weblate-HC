...
import { EmptyState, KindBadge } from "@/components/console/primitives";
...
const KIND_LABEL = {
  blocking: "Блокирует выгрузку",
  judge_critical: "Судья: критично",
  judge_note: "Судья: замечание",
};
...
function HighlightedSource({ text, ranges }) {
  ...
  return (
    <>
      {text.slice(0, r.from)}
      <mark className="rounded-sm bg-highlight-glossary px-0.5 text-foreground">
  ...
  const actions = [
    {
      ...
      primary: true,
      onClick: () => onRepair(decision),
    },
    {
      key: "weblate",
      label: "Открыть в Weblate",
      onClick: () =>
        navigate(`/advanced?url=${encodeURIComponent(decision.advanced_url)}`),
    },
  ];
  if (!isBlocking)
    actions.push({
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-body-sm text-muted-foreground">
              {decision.language_name} · {decision.language}
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem
                  onClick={() => {
                    navigator.clipboard?.writeText(decision.key);
                ...
                <DropdownMenuItem
                  onClick={() =>
                    navigate(
                      "/advanced?url=" +
                        encodeURIComponent(decision.advanced_url),
                    )
                  }
                >
        ...
        <div className="mt-6 space-y-4">
          <div>
            ...
            <div className="code-cell rounded-sm border border-border bg-muted p-3 text-foreground">
              <HighlightedSource
                ...
                ranges={decision.glossary}
              />
            </div>
          </div>
          <div>
            <div className="mb-1 text-label-caps uppercase text-muted-foreground">
              Перевод · {decision.language}
            </div>
            <div className="code-cell rounded-sm border border-border bg-card p-3 text-foreground">
              {decision.target}
            </div>
          </div>
          <div>
            ...
            {decision.back_translation ? (
              ...
              <div className="rounded-sm border border-dashed border-border p-3 text-body-sm text-muted-foreground">
                нет — появится после проверки судьёй
              </div>
            )}
          </div>
          ...
          {repairing && (
            <div className="rounded-sm border border-info-surface bg-info-surface p-3 text-body-sm text-info-strong">
              Строка отправлена на исправление. Результат появится после прогона
              — статус можно отслеживать в прогоне.
            </div>
          )}
          ...
          <div>
            ...
            <ol className="space-y-2">
              {decision.history.map((h) => (
                ...
                >
                  <span className="mt-0.5 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-muted text-label-caps text-muted-foreground">
                    {h.attempt}
                  </span>
                  <span className="text-muted-foreground">
                    <span className="font-mono">{h.kind}</span> — {h.note}
                  </span>
                </li>
              ))}
            </ol>
        ...
        <div className="mt-8 flex flex-wrap items-center gap-2 border-t border-border pt-4">
          {actions.map((a, i) => (
            <Button
              ...
              onClick={a.onClick}
            >
              <span className="me-1 inline-flex h-4 min-w-4 items-center justify-center rounded-sm bg-black/10 px-1 text-[10px]">
                {i + 1}
              </span>
        ...
        <Dialog open={acceptOpen} onOpenChange={setAcceptOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Укажите короткую причину — она сохранится в истории строки.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-2">
              <Label htmlFor="accept-reason" className="text-label-strong">
                Причина
              </Label>
              <Textarea
                id="accept-reason"
                ...
                placeholder="Например: осознанная стилистическая правка"
                rows={3}
                autoFocus
              />
            </div>
...
export function DecisionsQueueScreen({ project, initialQuery = {}, navigate }) {
  ...
  React.useEffect(() => {
    api.getDecisions(slug, {}).then(setAll);
  }, [slug]);
  ...
  React.useEffect(() => {
    pushUrl({ lang, kind, content, q });
  }, [lang, kind, content, q, pushUrl]);

  const matchKind = (d) =>
    !kind ||
    (kind === "blocking" ? d.kind === "blocking" : d.kind !== "blocking");
  const matchContent = (d) =>
    !content ||
    (content === "stores"
      ? d.content.startsWith("store")
      : d.content === "loc-kit");
  const matchQ = (d) => !q || d.key.toLowerCase().includes(q.toLowerCase());

  // biome-ignore lint/correctness/useExhaustiveDependencies: matchKind/matchContent/matchQ are pure closures over kind/content/q, already listed as deps; re-listing them would recompute on every render.
  ...
  const langCounts = React.useMemo(() => {
    ...
    const base = all.filter(
      (d) =>
        !removed[d.unit_id] && matchKind(d) && matchContent(d) && matchQ(d),
    );
    const m = {};
    base.forEach((d) => {
      m[d.language] = (m[d.language] || 0) + 1;
    });
    return m;
  ...
  const doRepair = async (d) => {
    ...
    toast(`Отправлено на исправление: ${d.key} · ${d.language}`, {
      duration: 6000,
    });
  };
  ...
  const doAccept = async (d, reason) => {
    ...
    toast(`Принято: ${d.key} · ${d.language}`, {
      duration: 6000,
  ...
  const bulkFix = async () => {
    ...
    for (const id of ids) {
      setRepairing((r) => ({ ...r, [id]: true }));
      await api.repairDecision(id);
    }
  ...
  // keyboard
  // biome-ignore lint/correctness/useExhaustiveDependencies: window keydown listener re-bound only on its state deps; doRepair/navigate capture only stable setters/navigator.
  React.useEffect(() => {
    const onKey = (e) => {
      if (e.key === "?") {
        setCheat(true);
      ...
      if (sheetOpen && active) {
        ...
        if (["1", "2", "3"].includes(e.key) && !typing) {
          ...
          if (e.key === "1") doRepair(active);
            ...
            navigate(
              `/advanced?url=${encodeURIComponent(active.advanced_url)}`,
            );
          else if (e.key === "3" && !isBlocking) {
            /* accept handled via dialog */
          }
        }
        return;
      }
  ...
  React.useEffect(() => {
    if (activeIdx >= 0 && rowsRef.current[activeIdx])
      rowsRef.current[activeIdx].focus();
  }, [activeIdx]);
    ...
    >
      ...
      <div className="mb-4 space-y-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Chip on={!lang} onClick={() => setLang("")}>
            Все
          </Chip>
          {project.languages
            .filter((l) => langCounts[l.code])
            .map((l) => (
              <Chip
                key={l.code}
                on={lang === l.code}
              ...
              >
                {l.name}{" "}
                <span className="opacity-70">{langCounts[l.code]}</span>
              </Chip>
            ))}
        </div>
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            ...
            <Chip on={kind === "blocking"} onClick={() => setKind("blocking")}>
              Блокируют выгрузку
            </Chip>
            <Chip on={kind === "judge"} onClick={() => setKind("judge")}>
              Судья
            </Chip>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Chip on={!content} onClick={() => setContent("")}>
              Всё
            </Chip>
            <Chip
              on={content === "loc-kit"}
              onClick={() => setContent("loc-kit")}
            >
              Лок-кит
            </Chip>
            <Chip
              on={content === "stores"}
              onClick={() => setContent("stores")}
            >
              Сторы
            </Chip>
          </div>
      ...
      {selectedCount > 0 && (
        <div className="mb-3 flex items-center justify-between rounded-sm border border-border bg-muted px-3 py-2">
          ...
          <Button className="h-8 rounded-sm" onClick={bulkFix}>
            <Sparkles className="h-4 w-4" />
            Исправить с помощью AI ({selectedCount})
          </Button>
        </div>
      )}
      ...
      {visible === null ? (
        ...
        <EmptyState
          icon={CheckCircle2}
          title="Всё решено"
          description={`${project.ready_count} из ${project.language_count} языков готовы к выгрузке.`}
          action={
            <Button
              className="h-9 rounded-sm"
            ...
            <tbody>
              {visible.map((d, i) => (
                <tr
                  ...
                  className={cn(
                    "h-10 cursor-pointer border-t border-border align-middle hover:bg-accent",
                    activeIdx === i && "bg-accent",
                  )}
                >
                  {/* biome-ignore lint/a11y/useKeyWithClickEvents: td is a container, not a control; the Checkbox inside handles keyboard, onClick only stops row-selection propagation */}
                  <td className="px-3" onClick={(e) => e.stopPropagation()}>
                    <Checkbox
                      checked={!!selected[d.unit_id]}
                      onCheckedChange={(v) =>
                        setSelected((s) => ({ ...s, [d.unit_id]: !!v }))
                      }
                      aria-label="Выбрать строку"
                    />
                  </td>
                  <td className="max-w-[180px] truncate px-3">
                    <span className="code-cell text-foreground">{d.key}</span>
                  </td>
                  <td className="whitespace-nowrap px-3 text-muted-foreground">
                    {d.language}
                  </td>
                  <td className="max-w-[260px] truncate px-3 text-muted-foreground">
                    <span className="code-cell">{d.source}</span>
                  </td>
      ...
      <StringSheet
        ...
        onOpenChange={(v) => {
          setSheetOpen(v);
          if (!v && active) rowsRef.current[activeIdx]?.focus();
        }}
        onRepair={doRepair}
        onAccept={doAccept}
