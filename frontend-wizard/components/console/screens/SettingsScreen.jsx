    ...
    <TooltipProvider delayDuration={150}>
      <div className="space-y-1">
        <div className="flex items-center gap-1.5">
          <Label className="text-label-strong text-muted-foreground">
            {label}
          </Label>
          <Tooltip>
            ...
            <TooltipContent>
              Настраивает администратор AI-инструментов
            </TooltipContent>
          </Tooltip>
        </div>
...
export function SettingsScreen({ project, navigate }) {
  ...
  React.useEffect(() => {
    api.getAdvancedLinks(slug).then(setLinks);
    api.getProfile(slug).then(setProfile);
  }, [slug]);

  const save = () => {
    api.patchProfile(slug, profile.answers).then(() => {
      setEditing(false);
      setConfirmOpen(false);
      toast("Профиль обновлён · кэш судьи обнулён");
    });
  };
  const setAns = (patch) =>
    setProfile((p) => ({ ...p, answers: { ...p.answers, ...patch } }));
      ...
      <div className="max-w-3xl space-y-6">
        <ConsoleCard title="Языки">
          <p className="mb-3 text-body-sm text-muted-foreground">
            Набор языков применяется ко всем загрузкам проекта.
          </p>
        ...
        <ConsoleCard title="Профиль проекта">
          {!profile ? (
            ...
            <div className="space-y-3">
              <WeblateLink
                url={`https://bdhc.hcgameloc.internal/titles/${slug}`}
                label="Открыть карточку игры в БДХК"
                navigate={navigate}
              />
              {!editing ? (
                <>
                ...
                <div className="space-y-3">
                  <div>
                    <Label className="text-label-strong">
                      Обращение в интерфейсе
                    </Label>
                    <div className="mt-1 flex gap-2">
                      {Object.entries(REG_UI).map(([k, l]) => (
                        <button
                          type="button"
                          key={k}
                          onClick={() => setAns({ register_ui: k })}
                          className={cn(
                            "rounded-sm border px-3 py-1.5 text-body-sm",
                            profile.answers.register_ui === k
                              ? "border-primary bg-primary text-primary-foreground"
                              : "border-border",
                          )}
                        >
                  ...
                  <div>
                    <Label className="text-label-strong">
                      Обращение в диалогах
                    </Label>
                    <div className="mt-1 flex gap-2">
                      {Object.entries(REG_D).map(([k, l]) => (
                        <button
                          type="button"
                          key={k}
                          onClick={() => setAns({ register_dialogue: k })}
                          className={cn(
                            "rounded-sm border px-3 py-1.5 text-body-sm",
                            profile.answers.register_dialogue === k
                              ? "border-primary bg-primary text-primary-foreground"
                              : "border-border",
                          )}
                        >
        ...
        <ConsoleCard title="Служебные параметры">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <LockedField
              label="Маршрутизация моделей"
              value="gpt-5.2 · claude-sonnet-4.5"
            />
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent>
          ...
          <p className="text-body-sm text-muted-foreground">
            Изменение профиля обновит промпты переводчика и судьи и обнулит кэш
            вердиктов судьи — следующая проверка качества будет платной для всех
            строк. Продолжить?
          </p>
          <DialogFooter>
            <Button
            ...
            <Button className="h-9 rounded-sm" onClick={save}>
              Продолжить
            </Button>
          </DialogFooter>
        </DialogContent>
