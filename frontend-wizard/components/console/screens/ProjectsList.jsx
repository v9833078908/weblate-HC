  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <h3 className="text-heading-card text-foreground group-hover:text-primary">
          {p.name}
        </h3>
        {p.decisions_count > 0 && (
        ...
        <>
          <dl className="mt-4 grid grid-cols-2 gap-y-3 text-body-sm">
            ...
            <dd className="text-end tabular-nums text-foreground">
              {p.ready_count} / {p.language_count}
            </dd>
            <dt className="text-muted-foreground">
              Потрачено в {p.month_label}
            </dt>
            <dd className="text-end text-foreground">
              <Money value={p.spend_month} />
            </dd>
          </dl>
    ...
    return [...projects]
      ...
      .sort(
        (a, b) =>
          ...
          b.decisions_count - a.decisions_count,
      );
  }, [projects, q]);
    ...
    <AppShell navigate={navigate}>
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Будет создан пустой проект. Локализацию вы настроите на
                  следующем шаге, загрузив лок-кит.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-2">
                <Label htmlFor="pname" className="text-label-strong">
                  Название проекта
                </Label>
                <Input
                  id="pname"
                  ...
                  placeholder="Например, Pirate Ships"
                  className="h-9"
                  autoFocus
      ...
      {sorted === null ? (
        ...
        <EmptyState
          ...
          description="По вашему запросу проектов нет. Измените запрос или создайте новый проект."
        />
      ) : (
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {sorted.map((p) => (
            <ProjectCard key={p.slug} p={p} navigate={navigate} />
          ))}
        </div>
      )}
    </AppShell>
