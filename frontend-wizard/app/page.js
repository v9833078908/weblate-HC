...
function useLocation() {
  ...
  React.useEffect(() => {
    const update = () => setLoc({ pathname: window.location.pathname, search: window.location.search })
    update()
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="flex">
        <div className="hidden w-60 shrink-0 border-e border-border bg-card p-3 md:block">
          {[0, 1, 2, 3, 4, 5].map((i) => <Skeleton key={i} className="mb-2 h-8 rounded-sm" />)}
        </div>
        <div className="mx-auto w-full max-w-[1200px] px-6 py-5">
          <Skeleton className="mb-6 h-8 w-48 rounded" />
          <div className="grid grid-cols-3 gap-6">
            <Skeleton className="h-28 rounded" /><Skeleton className="h-28 rounded" /><Skeleton className="h-28 rounded" />
          </div>
...
function ProjectGate({ slug, navigate, children }) {
  ...
  React.useEffect(() => {
    ...
    api.getProject(slug).then((p) => alive && setProject(p)).catch(() => alive && setError(true))
    return () => { alive = false }
  }, [slug])
  if (error) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4">
        ...
        <button onClick={() => navigate('/')} className="text-body-sm text-primary hover:underline">К списку проектов</button>
      </div>
    )
  }
...
function App() {
  ...
  if (parts.length === 0) {
    ...
    screen = <EmailPreviewScreen kind={parts[1] || 'completion'} navigate={navigate} />
    ...
    screen = (
      <ProjectGate slug={slug} navigate={navigate}>
        {(project) => {
          ...
          if (section === 'download') return <DownloadScreen project={project} navigate={navigate} />
          if (section === 'settings') return <SettingsScreen project={project} navigate={navigate} />
          if (section === 'runs') return <RunCardScreen project={project} runId={parts[3]} navigate={navigate} />
          return <ProjectOverviewScreen project={project} navigate={navigate} />
        }}
      </ProjectGate>
    )
  } else {
