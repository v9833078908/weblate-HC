...
языков. Для pairs она допускается только на строках терминов: текст в той же
колонке на строке описания или caption-строке отклоняется до публикационного
гейта, потому что parser не может объявить такую ячейку.
