...
Дата: 2026-08-13, пересмотрен в тот же день по итогам B2'; поправки по
полному замеру рекалибровки severity внесены 2026-08-19. Статус:
**дизайн согласован, судьи выбраны замером, реализация не начата;
severity-гейт пересмотрен по R3 — см. датированные поправки.**
...
- `docs/llm-first/vision/llm-first-product-architecture.md` (часть 4)
...
- `docs/llm-first/research/2026-08-11-judge-ux-competitor-research.md`
- `docs/llm-first/measurements/2026-08-13-phase0-measurements.md` (замеры фазы 0)
- `docs/llm-first/measurements/2026-08-19-severity-recalibration-final.md` (полный замер n=5, гейты R1-R3)

Состояние кода: **судьи в коде нет**, есть калибровочные артефакты в
`analysis/data/` (золотой набор `col4-judge-golden.json`, 919 записей;
разметка B0, 560 юнитов) и закрытая калибровка B2'
(`docs/llm-first/measurements/2026-08-13-phase0-measurements.md`). Она заменила
...
| Что с неразрешённой строкой | по severity: critical удерживается до решения человека, остальное отгружается (пересмотр 2026-08-19, см. ниже) |
...
Замер 2026-08-13 (`docs/llm-first/measurements/2026-08-13-phase0-measurements.md`)
...
> **Поправка 2026-08-19.** «0 из 120 critical в auto-pass» измерено на
> n=2 повтора и не подтвердилось. Полный замер рекалибровки
> (`2026-08-19-severity-recalibration-final.md`, n=5 × 2 судьи, эталон —
> человеческая 4-уровневая разметка 124 юнитов zh_Hans): **ни один
> прогон коллегии не поймал все 7 настоящих critical** (пропуски 1-5 за
> прогон во всех плечах), ложных critical 4-5/124 по медиане при пороге
> гейта ≤2. Гейт R2 не пройден ни одним промптом; по правилу R3
> severity-гейт как *автоматическая гарантия* отзывается. Механика
> `critical → state 10` остаётся, но её смысл — **очередь на решение
> человека** (4-9 строк на 124 — объём, который продюсер реально
> разбирает перед билдом), а не гарантия «сломанное не уйдёт». Это
> возвращает дизайн к инварианту 4.1.2 архитектуры: вероятностный
> вердикт не блокирует релиз, блокируют детерминированные проверки и
> человек.
...
> **Поправка 2026-08-19.** Регулятор измерен на zh-срезе и **отвергнут**:
> «повышение до critical» вторым судьёй — самый шумный его сигнал
> (медиана ложных critical 4 за прогон), а его вклад в recall живёт в
> `flag`-вердиктах, которые регулятор отбрасывает. Конфигурация хуже
> полной коллегии по обеим осям; не включать.
...
> (`docs/llm-first/measurements/2026-08-14-st2-zh-judge-run.md`, раздел «Второе
...
> `docs/llm-first/plans/2026-08-14-judge-severity-recalibration.md`.
...
`docs/llm-first/measurements/2026-08-14-st2-zh-judge-run.md`). Реализации требования
...
(`analysis/probes/col4-judge-eval.py`, `reply_schema()`), и вся смысловая
...
4. **Отнести к следующему срезу.** Изменение промпта существенное,
   ...
   (`docs/llm-first/measurements/2026-08-13-phase0-measurements.md`, пункт 7
   рекомендаций), поэтому цена правки — новый запечатанный срез.

> **Поправка 2026-08-19.** Пункты 1-2 измерены на новом запечатанном
> срезе (`2026-08-19-severity-recalibration-final.md`): обязательный
> `description` **не улучшает метрики судьи** (R1 не пройден: точность
> 0.51→0.54, но recall 0.75→0.67) — его ценность только в уликах для
> продюсера. Промпт по умолчанию для реализации — **плечо D** того
> замера: `description` + рубрика severity «последствие для игрока»
> (плечо C) **плюс** рендер-превью плейсхолдеров во входе. Плечо C —
> единственное без проигрыша по шуму и precision (шум коллегии −23%,
> 39→30/124; REAL@14 12/14; ложные флаги deepseek вдвое ниже, 8→4/83;
> precision 0.51→0.56 при recall внутри шумового пола), а рендер-превью
> поверх него поднимают плейсхолдер-critical (24207 0/5→4/5, 24130
> 2/5→5/5, ячейка `critical→none` 4→0) при плоском шуме — см. раздел
> «Рендер-дефекты плейсхолдеров» и приложение к финальному замеру.
...
> **Поправка 2026-08-20 (соседние реплики диалога).** Пункт «показываем,
> что судья видел» был неполным: сегмент судьи не содержал соседних реплик
> диалога — ни в payload (`weblate/trans/judge.py::_segment`: `id, key,
> source, target, rendered_*, note, glossary, checks`), ни в протухании
> (`compute_context_hash` = `source + note + glossary_terms`). Сегменты
> режутся по `JUDGE_BATCH_SIZE = 5` подряд, но батч — транспортная единица,
> а не диалоговый контекст: модель не уведомлена, что строки
> последовательны, и на границах батчей контекст исчезает.
> Наблюдённые промахи (аудит heart-abyss/hub-1, `misc/heart-abyss-hub-1-translation-qa.md`):
> `hub1_teahouse_1_11` — обрывок фразы, чей смысл даёт непосредственно
> предыдущая реплика `hub1_teahouse_1_10` (окно ±1 закрывает), и
> `hub1_ramen_1_13` («остановиться» во фр. передано как «arrêter» =
> «притормозить» вместо «остановиться на ночлег»; смысл задаёт
> «Нам нужна комната» в `hub1_ramen_1_18` — пять реплик дальше, окно ±1
> её **не** закрывает). Оба — ложные срабатывания от изоляции сегмента:
> судья физически не видел строк, дающих смысл. Первый пример закрывается
> окном ±1, второй показывает, что минимального окна может не хватить —
> поэтому решение принимается замером (плечо H2), а не анекдотом.
> Класс ошибки затрагивает связный диалог; однострочные UI-строки им не
> затронуты.
>
> Решение: контекст диалога — **поле сегмента, а не порядок батчей**.
> `context: {prev_source, next_source}` (только source-строки соседей;
> соседние target — отдельная гипотеза, не входящая в этот срез),
> передаётся независимо от того, попал ли сосед в тот же батч. Соседние
> реплики — справочный материал для связности, не текст для перевода и не
> источник ошибок (зеркало формулировок про `glossary` и `checks`).
> `compute_context_hash` обязан включать соседние source, иначе вердикт
> останется «свежим» после правки соседней строки — тот же класс
> протухания, под который hash проектировался.
>
> По правилу R3 изменение payload и промпта одним срезом с повторным
> замером: плечо H2 в драйвере `analysis/probes/st2-zh-recalibration.py`
> (контекст соседей в payload + строка в промпт, без прочих правок),
> гейты — план `docs/llm-first/plans/2026-08-20-judge-dialog-context.md`.
>
> **Индустрия подтверждает приём.** Замер hub-1 (§15.1 того же аудита,
> продовый вход судьи, 3 прогона коллегии) показал: 4 дефекта из 14
> ручных не взял даже union трёх прогонов, и **три из четырёх — класса
> «нужна соседняя реплика»** (`собрат→frère`, `остановиться→arrêter`,
> `за мой счёт→pour ma pomme`). Внешнее исследование девяти TMS (§16)
> нашло ровно одного вендора с штатным решением: **Crowdin AI QA
> подставляет в промпт `%siblingsStrings%` — предыдущую и следующую
> строки**; единственный найденный вендорский приём, прямо бьющий в
> измеренный потолок судьи. У нас эквивалент — расширение
> `build_request` (`weblate/trans/judge_loop.py:70`): соседи по
> `Unit.position`, один и тот же прием для перевода и суда.
>
> **Итог 2026-09-04.** Плечо H2 измерено на 109 диалоговых юнитах hub-1
> против продовых сидений
> (`docs/llm-first/measurements/2026-09-04-judge-dialog-context-paired.md`)
> и отклонено: seat 2 не меняет ни одного вердикта, seat 1 - в пределах
> шума; `teahouse_1_11` остаётся critical при дословно поданном соседе,
> `собрат→frère` seat 2 ловит и без соседей. «Изолированный сегмент»
> остаётся известным ограничением судьи; показывать соседние реплики
> человеку в UI - отдельный продуктовый вопрос, не LLM-контекст.
...
> **Поправка 2026-08-19.** Таблица и механика остаются, но строка
> `critical → 10 → нет` больше не читается как гарантия: полный замер
> показал, что severity судьи ловит не все настоящие critical (1-5
> пропусков за прогон) и даёт 4-5 ложных на 124. `state 10` — это
> **очередь на решение человека** с удержанием старой версии строки, а
> экран готовности (план 2) с числом удержанных строк из nice-to-have
> становится обязательной частью гейта: релизный гейт = детерминированные
> чеки + разобранная человеком critical-очередь.
...
### Рекомендация к рассмотрению (2026-08-20): воронка допуска вместо чтения панели

Происхождение: аудит `heart-abyss/hub-1`
(`misc/heart-abyss-hub-1-translation-qa.md`, раздел 12) — 0 approved,
0 suggestions, 0 comments; панель чеков не читает никто, и чек,
стреляющий в непрочитанную панель, стоит ноль. Рекомендация собирает
решения этого документа в воронку, где панель потребляется процессом,
а не глазами. Статус — к рассмотрению вместе с открытым вопросом выше;
решений документа не меняет, добавляет порядок включения.

1. **`commit_policy = WITHOUT_NEEDS_EDITING` — явное предусловие
   гейта.** Документ уже опирается на эту политику (раздел «Гейт по
   severity выражается штатными настройками»), но дефолт поля — `ALL`
   (`weblate/trans/models/project.py:341`): пока политика не
   переключена на проекте, и enforced-провал (11), и critical (10), и
   fail-safe (10) меняют статистику, а не файл. Переключение — одна
   настройка проекта, дешевейший ход всей воронки; фиксировать его как
   шаг раскатки, а не подразумевать.

2. **`enforced_checks` на игровых компонентах — ярус «факт» до суда.**
   Четыре детерминированных zero-FP чека (`game-markup`,
   `game-line-break`, `game-number`, `game-token`) в
   `Component.enforced_checks`: провал → state 11 → строка не готова и
   не уезжает. Инертны там, где в исходнике нет их паттернов. Порядок:
   сначала замер ретроактивной демоции (`update_enforced_checks`,
   `weblate/trans/models/component.py:6751`, демотирует уже
   переведённые юниты), потом включение; для новых компонентов —
   предустановка набора в `create_component_from_kit`
   (`weblate/utils/views.py`), чтобы «забыли включить» не существовало
   как состояние. Судейские `judge-*` в enforced не попадают никогда —
   инвариант 1 уже страхуется кодом
   (`weblate/trans/models/unit.py:2454` вычитает `JUDGE_CHECKS`).

3. **Producer-only ответ на открытый вопрос (предложение).**
   `translation_review=True`, но носители review-права — бот судьи
   (state 30 по `pass`) и один человек (critical/exhausted, замеренный
   объём 4-9 строк на 124). Ложная очередь человека не возникает:
   штатный столбик `unapproved` разбирает режим `judge`, а не люди;
   массовый bulk-approve на релизном срезе не нужен вовсе. Человеческая
   очередь — закладки `?q=has:check` и судейские фильтры плана 2;
   Zen-режим этих же ссылок — очередь триажа.

4. **`RepeatDriftCheck`**
   (`docs/llm-first/plans/2026-08-14-intra-component-consistency-check.md`)
   встраивается advisory-чеком в ту же `has:check`-очередь: находка не
   блокирует сборку, но юнит не пройдёт в approved мимо человека —
   исправление или dismiss с аудит-следом (`has:dismissed-check`).
   Замер 2026-08-20 по проду: 35 групп дрейфа на hub-1+temple только в
   EN+FR, 0 из них видимы стоковым `inconsistent`.

Воронка целиком: LLM (fail-safe 10) → enforced-чеки (факт, 11) →
коллегия (мнение: pass 30 / major 20 + чек / critical 10) → человек
(решение с причиной). Гейт = `WITHOUT_NEEDS_EDITING` + разобранная
critical-очередь — формула поправки 2026-08-19 без изменений; воронка
добавляет к ней порядок включения настроек и место детерминированных
чеков до суда.
...
(см. преамбулу `docs/llm-first/plans/2026-08-12-phase0-implementation.md`).
...
| Backfill автофиксов — код | layer0-план + `docs/product/plans/2026-08-13-autofix-backfill-scoped-commit.md` | **отгружено**: команда `reapply_autofixes`, scoped commit по корню репозитория, row locks, `autofix_fingerprint()` | закрыт |
...
(`docs/llm-first/measurements/2026-08-13-phase0-measurements.md`) отверг каскад:
...
(`docs/llm-first/plans/2026-08-11-glossary-morphological-enforcement.md`), после
...
**Рендер-дефекты плейсхолдеров: превью во входе судьи их поднимают.**
Юнит 24207 (`ID_AI_PLAYER_RANK_STATUS_NOTIFICATION_DESC`) и класс
инверсии ролей (`{[PARAM0]}`/`{[PARAM1]}`, 24130) существуют только в
отрендеренной строке — судья, видящий сырую строку с `{0}`/`{1}`,
систематически их терял (24207 — ни разу не critical за n=5 плеча C).
Формулировкой промпта это не лечится (R3), но **входом — да**: плечо D
(детерминированная подстановка примерных значений в плейсхолдеры,
отрендеренная пара в промпт) при коллегии n=5 подняло 24207 до
критического в 4/5 прогонов, 24130 — в 5/5, а ячейку `critical→none`
обнулило (4→0). Замер: приложение к
`2026-08-19-severity-recalibration-final.md`. Отсюда два переноса:
рендер-превью в **вход судьи по умолчанию** (плечо D, дёшево) и в **UI
юнита для продюсера** (та же подстановка человеку). Это не гейт: гейт R2
плечо D не спасло (false_crit 5→6, не все прогоны берут все critical), и
часть дефектов (омиссии) к рендеру безразлична — детерминированная
рендер-проверка на стороне движка остаётся отдельной задачей.
...
|---|---|---|---|
| **1. Вердикт** | `docs/llm-first/plans/2026-08-13-01-judge-verdict-core.md` | `JudgeVerdict` + миграция, классы judge-чеков, клиент судьи (промпт — плечо D замера 2026-08-19), коллегия двух судей и починка в Celery, режим `judge` в `AutoForm` + чекбокс перезаписи, fail-safe state 10, удержание critical до решения человека, карточка на юните, обратный перевод в форме, протухание, исключение судейских чеков из «Things to check» | прогон на фильтре даёт построчный вердикт; `critical` не уходит в сборку без решения человека; навигация через `check:judge-*` |
| **2. Навигация и готовность** | `docs/llm-first/plans/2026-08-22-02-judge-navigation-readiness.md` | `judge_field` в `search.py`, регистрация в `FILTERS`, ключи в выпадашке формы, `calculate_judge()`, карточка «Оценка ИИ» на обзоре, баннер готовности, таблица по языкам, `NOT has:judge` как дефолтный `q` режима `judge`, счётчик строк и оценка цены под фактический `q` формы | на вопрос «можно ли отгружать билд» отвечает один экран; `NOT has:judge` показывает непокрытое; счётчик обещает ровно то, что прогон осудит |
| **3. Решения и белый ящик** | `docs/llm-first/plans/2026-08-24-03-judge-decisions-and-whitebox.md` | кнопки решений с обязательной причиной, `resolution` в работе, вкладка «Оценка ИИ» с историей суда, парсер судейских спанов в `Formatter`, отдельный класс бейджа | продюсер закрывает петлю с аудит-следом; видно, почему судья решил так |
...
- **До плана 2 счётчик строк в форме считает не то, что будет осуждено.**
  Подпись обещает «The current filter matches N strings», а `judge_row_count`
  берёт фиксированный `state:<translated`
  (`weblate/trans/views/basic.py:805-812`) — не тот `q`, что стоит в форме, и
  даже не её собственный дефолт `state:empty` (`weblate/trans/forms.py:1201`).
  Оценка цены умножает это же число. На аудитном прогоне по уже переведённым
  строкам форма показывает «0 строк» и затем судит их все, то есть занижает
  ровно в том случае, ради которого счётчик и заведён — показать цену до
  траты денег (замер: `docs/llm-first/measurements/2026-08-20-judge-first-dev-run.md`).
  Правильный дефолт и правильный счётчик приходят одним куском в плане 2:
  оба упираются в `has:judge`.
...
- **Оба судьи ошибаются одинаково.** Коллегия работает ровно потому,
  ...
  надо перемерять, а не наследовать. Согласие мест — не гарантия, а
  измеренное свойство конкретной конфигурации.
  **Риск реализовался 2026-08-14** на другой паре языков: на `zh_Hans`
  `deepseek` за два прогона не нашёл ни одного дефекта сверх двух
  прогонов `qwen` — уникальный вклад второго места оказался нулевым,
  а не 34 дефекта, как на `fr`. Свойство не переносится между
  корпусами и парами языков; см.
  `2026-08-14-st2-zh-judge-run.md`.
- **Severity-шкала судьи не калибрована против человеческой.** RUBRIC-MQM
  ...
  на другом корпусе или при смене модели.
  **Риск подтверждён замером 2026-08-19 с обеих сторон**
  (`2026-08-19-severity-recalibration-final.md`): baseline severity
  сломана количественно (25/415 наблюдений чистых строк получили
  critical; 7/35 настоящих critical занижены), а LLM-аннотаторы
  (`claude-sonnet-5`, `gpt-5.4-mini`) воспроизводят те же слепые пятна —
  9 из 14 человеческих дефектов занижены обоими. Мера уточнена: severity
  agreement меряется **только против человеческой разметки**; модельные
  аннотаторы годятся как перекрёстная проверка полноты, но не как эталон.
...
- **Граница ответственности судьи по глоссарию.** Проверено по
  `analysis/data/col4-judge-golden.json`: все 195 записей с
  ...
  `docs/llm-first/measurements/2026-08-11-glossary-enforcement-analysis.md:140-170`):
...
- **Отпечаток не покрывает глоссарный матчер.** Метаданные набора пиннят
  ...
  `docs/llm-first/plans/2026-08-11-glossary-morphological-enforcement.md` — меняет,
