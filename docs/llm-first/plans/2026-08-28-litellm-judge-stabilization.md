...
**Дата:** 2026-08-28. **Статус:** задачи 1-5 реализованы и работают на проде;
...
(строка 282 ниже) и реализовано: `http-auth` поднимает `JudgeError`
(`weblate/trans/judge.py:1516-1517`). Для `http-other` (400/404/422) такого
правила нет, поэтому ран 2026-09-01 05:59 **завершился успешно**, отдав 50
подряд отказанных батчей с HTTP 400 и записав 50 `unparsed`-вердиктов вместо
остановки после первого отказа. 101 из 102 диагностируемых `unparsed`-вердиктов
за всё время - это именно отказ запроса (400/401), а не ответ модели.

**Следствие для задачи 6.** Включать очередь до закрытия этого пробела нельзя:
`_sync_deferral` ставит в очередь любой `result.unparsed` без учёта failure kind
(`weblate/trans/judge_loop.py:779-795`), поэтому при
`JUDGE_DEFERRAL_ENABLED=1` инцидент 05:59 превратился бы в периодическую
ретрай-петлю против endpoint'а, который отказывает всегда, и закончился бы 51
строкой `slow` вместо 51 терминальной.
