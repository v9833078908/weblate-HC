...
потоковым агрегирующим запросом на языковую группу, загружает кандидаты
ограниченными порциями и решает расхождение в питоне;
...
до следующего батч-прохода; после коммита группа повторно сверяется под
проектным lock, чтобы параллельные правки не оставили устаревшие строки.
`audit_glossary` — read-only management-команда
...
Статус: **реализован 2026-09-03, выключен по умолчанию.** Не задеплоен нигде.
Включение на проде требует
...
- 2026-09-03: `RepeatDriftCheckTest` и `AuditGlossaryCommandTest` — 82
  passed.
- 2026-09-03: `pytest weblate/checks weblate/glossary weblate/machinery
  weblate/trans/tests/test_models.py` — 3 009 passed, 551 skipped, 47 subtests
  passed. Шесть не связанных с изменением тестов custom OpenAI/Mistral
  завершаются ошибкой DNS при валидации внешних тестовых URL.
- 2026-09-03: scoped `prek` прошёл все проверки затронутых файлов, кроме
  глобального `typos`: он сканирует весь репозиторий и сообщает о существующих
  многоязычных файлах корпусов вне этого изменения. Pylint затронутых модулей —
  10.00/10. Mypy сообщает о существующих ошибках в модулях и их зависимостях,
  но ни одна не указывает на изменённые строки.
- Сборка документации не запущена: в окружении нет Graphviz `dot`; dev-контейнер
  и прод намеренно не трогались.
