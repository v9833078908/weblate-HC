...
> **Пересмотрено 2026-08-13.** Операционный документ по судье —
> `2026-08-13-judge-native-ui-design.md`. Он заменяет здесь восемь
> решений: хранилище вердикта и состояние при flag (4.3), релизный
> гейт (4.5), 3x-агрегацию (4.2), MVP «ноль правок шаблонов» (4.4),
> пер-языковые тиры и блокер CometKiwi (4.6), протокол калибровки B2,
> точку входа (внешний пайплайн над REST -> режим формы автоперевода).
> Полный список с обоснованием — в его разделе «Расхождения с
> существующими документами».
>
> Девятое, добавленное после закрытия B2': **глоссарные дефекты судье
> не принадлежат.** Судья собирает глоссарий тем же
> `get_glossary_terms()`, что и переводчик, поэтому невидимый матчеру
...
eval-харнесс (`analysis/probes/col4-eval-harness.py`) с базлайном и замером
...
Дополнено 2026-08-12: `c26a967` и `0549923` (оба 2026-08-11, после
написания этого раздела) отгрузили три пункта из списка выше раньше,
чем статус был обновлён. json_schema — нестрогий вариант (без
`strict`, корень схемы — массив, без юнит-тестов, безусловно на каждый
запрос), не strict+конверт дизайна трека A. Few-shot — уже только из
`STATE_APPROVED`, либо из последнего не-автоматического изменения, если
...
- **Соседние реплики в контексте судьи** —
  `docs/llm-first/plans/2026-08-20-judge-dialog-context.md` (статус: ожидает
...
- **`RepeatDriftCheck` и аудит живого глоссария** —
  `docs/llm-first/plans/2026-08-14-intra-component-consistency-check.md` (статус: не
  начат). Детерминированный TargetCheck: одинаковый исходник в одном проекте
  и языке обязан иметь одинаковый перевод (на col4/data/fr 57.5% групп
  повторов разошлись, стоковый `inconsistent` не пометил ни одной; замер
  `docs/operations/measurements/2026-09-03-need-for-greed-repeat-drift.md`
  нашёл 122 группы без глоссария). Дрейф повторов — сравнение, а не понимание
  смысла, поэтому слой принадлежит коду, а не судье, который дрожит на
  22-31% идентичных прогонов.
