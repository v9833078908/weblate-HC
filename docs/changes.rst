...
* Every LLM judge producer launch is now recorded as one durable, permission-checked run report, addressed by its own URL and independent of the Celery task result that started it; it lists checked, cached, skipped, repaired, and per-outcome counts, each opening the matching strings, and a finished component, project, or translation page links to its own last run, see :ref:`llm-judge`.
