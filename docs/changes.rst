...
* Every LLM judge producer launch is now recorded as one durable, permission-checked run report, addressed by its own URL and independent of the Celery task result that started it. The report opens on the strings that still need a producer's decision - held critical rows, unresolved majors and minors, and any string the judge could not finish - grouped by the error category the judge assigned, worst problem first, with every other outcome one click away; a finished component, project, or translation page lists its recent runs behind a :guilabel:`Latest report` button with the earlier ones in a menu, see :ref:`llm-judge`.
