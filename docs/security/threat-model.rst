...
.. list-table::
   ...
   * - LLM judge
     ...
     - Database (verdicts; each resolution transition permanently appends
       ...
       outcomes are database-only records with no outbound leg; the run and
       its aggregates are readable through the permission-checked report
       page, and a string's own stored verdict is additionally rendered to
       users who may view that translation. The report never renders a cost
       figure or an identity hash that could confirm a known translation
...
.. list-table::
   ...
   * - Loc-kit glossary draft to OpenRouter
     - An authenticated table upload becomes a fixed outbound request only
       ...
       :doc:`/admin/config`)
   * - Judge string batch to the configured provider
...
off by default, its key and provider endpoint are single site-wide
configuration rather than per-project configuration, the key is built inline
