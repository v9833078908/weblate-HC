...
```python
repairs = (
    repair_targets(repairable_units, user, run_id=run_id) if repairable_units else {}
)
```
...
unpriced_requests = translation_spend.unpriced_requests + judge_spend.unpriced_requests
cost_per_written = (
...
def test_translation_run_report_breaks_spend_down_by_provider_language_model(
    ...
    self.assertEqual(
        [
            (row["language"], row["service"], row["model"], row["cost_usd"])
            for row in rows
        ],
        [
...
def test_non_judge_task_result_links_to_its_report(self) -> None:
    ...
    self.assertEqual(result["report_url"], reverse("judge-run", kwargs={"pk": run.pk}))
...
def test_history_menu_lists_a_translation_run(self) -> None:
    ...
    self.assertIn(
        run.pk,
        [item.pk for item in recent_producer_runs(self.component, user=self.user)],
    )
```
...
if mode != "judge" and autoform.cleaned_data["auto_source"] == "mt" and engine_ids:
    ...
    if complete and per_string:
        pretranslation_cost = {
            "available": True,
            "min": format((min(per_string) * mt_preview.writable).normalize(), "f"),
            "max": format((max(per_string) * mt_preview.writable).normalize(), "f"),
        }
```
