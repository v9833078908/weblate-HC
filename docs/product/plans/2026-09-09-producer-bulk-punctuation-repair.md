...
- При существующем GET анализа создать короткоживущий snapshot в общем Django cache, TTL равен `PENDING_TASK_MAX_AGE` из `weblate/utils/celery.py` (сейчас 1 800 секунд), UUID + Django-signed handle и абсолютный `start_deadline`. Запись в cache не меняет переводов и не запускает задачи. Pagination использует тот же handle; нельзя пересчитывать новый набор незаметно на каждой странице. POST и worker не могут продлить deadline: задача, потерявшая PENDING-индикатор после 30 минут, не должна начать запись позже.
...
- Нет/expired/evicted snapshot или истёк `start_deadline` при POST/старте worker — **ничего не применять**, предложить заново открыть анализ. Worker загружает снимок один раз перед работой, проверяет deadline, сразу публикует PROGRESS до записи. Начатое вовремя выполнение может завершиться после preview TTL; это не разрешает поздний старт/повторную доставку. Автоочистка TTL принадлежит cache, новых cleanup tasks нет.
...
- Перед первой записью атомарно пометить существующий snapshot как `mutation_started`, сохранив данные и исходный `start_deadline`; срок хранения snapshot/claim продлить до текущего lock lease и обновлять вместе с heartbeat, не продлевая разрешение на новый старт. Marker не отдельный вытесняемый ключ. Ошибка/исчезновение cache — остановка до записи, никогда создание заново под прежним UUID. Повторная доставка после перехода не продолжает mutation и сообщает unknown/failed. Retry допустим только до mutation и до deadline; потеря lease — остановка без захвата чужого lock. Новый анализ уже исправленных строк идемпотентен.
...
Финальный security-review выявил один дополнительный дефект плана (`PLAN-REPAIR-001`): одночасовой snapshot мог запустить mutation после того, как существующий UI уже скрывал PENDING задачу через 30 минут. Исправлено: TTL/start deadline связаны с `PENDING_TASK_MAX_AGE`, а worker немедленно публикует PROGRESS. Regression для задачи B: просроченный PENDING запуск после reload не меняет ни одной строки; своевременно начатая задача остаётся наблюдаемой и не разрешает повторную mutation при redelivery. Deadline и время хранения started-marker различаются намеренно.
