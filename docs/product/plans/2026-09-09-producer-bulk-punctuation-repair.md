...
**Дата:** 2026-09-09. **Статус:** реализовано на ветке `feat/producer-bulk-punctuation-repair` (Tasks A-E), проверено, **на прод не задеплоено** — deployment и запись переводов на проде требуют отдельного явного одобрения.
...
Критерий покрытия проверен на **Anvil Saga вместо CoL4**: сохранённой локальной выборки CoL4/fr в репозитории нет, а пользователь в сессии 2026-09-09 прямо перенаправил проверку на прод-проект `anvil-saga` (read-only). Результат: 80-81 % целевых строк с чеками автоприменимо (порог — 50 %; интервал, а не точка, потому что пересечение автоприменимых строк терминальной и механической политик не снято — сами пулы пересекаются на 70 из 76 механических строк). Замер и его границы — `docs/operations/measurements/2026-09-09-anvil-saga-bulk-repair-projection.md`. Критерий про выборку `CoL4/data/fr` (2 092 / 7) этим замером **заменён, а не выполнен**.

Не сделано осознанно: применение политики на проде, деплой, разбивка `manual` по причинам отказа, живая приёмка механических групп (в корпусе Anvil для них почти нет целей — см. «границы» в замере).
