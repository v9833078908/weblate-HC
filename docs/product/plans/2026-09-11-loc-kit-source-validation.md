...
2. валидность тега определяется **собственной строгой грамматикой** модуля,
   ...
   TAG_WELL_FORMED = r"""
     <
     (?P<close>/)?
     (?P<name>color|link|size|b|i|u|s|sprite)
     (?:
         = (?P<value>[^\s<>=]+)          # <color=#25E94F>, <size=14>
       | \s+ name="(?P<attr>[^"<>]+)"    # <sprite name="fire">
     )?
     /?>
   """
   ```
...
3. `source.tag_closing_has_attribute` — закрывающий тег (`close` не пуст),
   у которого есть `value` или `attr`: ровно случай 525591. Считается
   отдельно от `tag_malformed`, потому что тег синтаксически корректен, а
   семантически невозможен;
4. `source.tag_unbalanced` работает на последовательности well-formed тегов:
   стек по имени, ошибка на закрывающем без открытого и на непустом стеке в
   конце строки. Самозакрытые (`<sprite name="fire">`, `<br/>`) в стек не
   кладутся;
5. `source.placeholder_unclosed` игнорирует строку, где нет ни одного
