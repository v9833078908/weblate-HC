...
Дата: 2026-09-17.
Статус: утверждён для исполнения; реализация в работе в ветке
``feat/repeat-drift-reconciliation``. Фактический статус и доказательства
проверок ведутся в конце документа; не считать незакрытые пункты выполненными
по наличию моделей или экранов.
...
**Статус: в работе.** Созданы policy/group/membership-модели, миграции и
проверка актуальности после unit/label/delete событий; покрыты точные группы,
stale identity и динамическое пересечение меток. Ещё требуется полная
сквозная проверка import/loc-kit и закрытых компонентов.
...
**Статус: в работе.** Реализованы подписанный preview, частичное применение,
история и guarded undo. Остаются contract tests для всех per-recipient
autofix/DOS/plural-случаев и замер scoped recheck.
...
**Статус: в работе.** Очередь Variant C, фильтры, статусы, preview,
редакторский/API/bulk-suggestion guard уже есть; нужны оставшиеся Zen/check
links, полный browser smoke и все accessibility-сценарии.
...
**Статус: в работе.** Есть durable run/attempt/result ledger, cap, strict
parser, usage и неизвестная доставка. Не закрыты scope/cost preview,
разбиение большого payload и отдельные task suites.
...
**Статус: в работе.** Принятый shared target переиспользуется до machinery
lookup; reservation одного generated candidate и полный run report ещё нужны.
...
**Статус: в работе.** Independent membership исключается из advisory check;
документация и changelog начаты. Остаются полные check/MT suites и
