...
Статус: **утверждён владельцем 2026-09-17 и исполняется.** Решения по
разделу 8: бюджет — лимиты итерации 1 ($5/слот, 50 POST/слот, 10 мин/слот,
residual risk принят); кандидаты — **только K1** (сравнить с результатами
итерации 1); batch seat 2 в стрессе — 5; корпус задачи 3 — QA-fixture
контролируемый, как в итерации 1 (вариант (a), честная пометка; вариант (b)
остаётся путём к production-adopt). Гейт-политика агрегации пары для
задачи 4 (консервативная решает + collegium публикуется) — подтвердить у
владельца до регистрации эксперимента задачи 4. Задачи 1–2 исполнены
(см. «Статус задач 1–2» в конце).
...
Следующие шаги: задача 3 (QA-fixture корпус качества с контролируемой
разметкой, срез для задачи 5 зарезервировать в сплите), задача 4 (контроль
— исходная пара против K1, обе агрегации), задача 5 (связка K1 + Qwen
batch 5 против исходной пары на неиспользованном срезе), задача 6 (отчёт и
решение). Временный wrapper `analysis/data/judge-cost-latency-ab/run-paid-slot.py`
(gitignored) воспроизводит паттерн обёртки раздела 4.
