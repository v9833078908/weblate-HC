...
- Create: `analysis/probes/glossary-terminology-fix.py`
...
Run: `uv run prek run --all-files --files analysis/probes/glossary-terminology-fix.py`
Expected: PASS. `analysis/probes/**.py` is not in `REUSE.toml`, so if `reuse` fails, add
`"analysis/probes/**.py"` to the HCGameLoc annotation block in `REUSE.toml` (next to
`analysis/data/**.csv`, `REUSE.toml:33`) and rerun.

**Step 4: Commit**

```bash
git add analysis/probes/glossary-terminology-fix.py REUSE.toml
...
```bash
payload=$(base64 < analysis/probes/glossary-terminology-fix.py | tr -d '\n')
