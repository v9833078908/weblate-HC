...
**Исполнителю.** Задачи 1 → 2 → 3 → 4 строго последовательны, и
параллельно с первым планом этот исполнять нельзя: общие
`autotranslate.py` (`perform`/`_perform`, `fetch_mt`, финализация
summary и warnings), точки входа `tasks.py`, `models/judge.py`, отчёт и
`ProducerRunSerializer` требуют одного владельца за раз. Из первого плана
сюда приходят готовыми: guard «одна доставка — один исполнитель»
(перезапись `_perform` обязана его сохранить), агрегат покрытия с
разбивкой по `skip_reason` и неизменная семантика статусов. Работать в
...
Исполнять через `executing-plans` в изолированном worktree.
