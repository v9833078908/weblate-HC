...
Дата: 2026-09-17.
Статус: план подготовлен к согласованию; реализация не разрешена и не начата.
Деплой, миграции и платные проверки требуют отдельного разрешения.
...
уже отправленные запросы не отменить задним числом. При наличии законченной
judge-работы это partial, иначе failed. Не держать DB-lock на время HTTP.
...
leaf; старый план с номером 0130 неприменим, этот номер уже занят.
...
- [ ] Добавить C5 snapshot/state, атомарное резервирование и новую причину
      skipped; финализировать failed/partial/cancel без зависших PENDING.
...
7. Перед следующей отправкой target стал empty: новые judge batches
   остановлены, уже завершённые сохранены, truthful partial/failed.
...
Планирование не является разрешением на реализацию. После согласования
