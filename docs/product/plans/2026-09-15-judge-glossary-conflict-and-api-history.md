...
**Дата:** 2026-09-15.
**Статус:** задачи 1, 3–8 и 10 реализованы; задача 1 смерджена и развёрнута, задачи 3–8 и 10 реализованы в ветке `feat/judge-proposal-only` (`b05f04d9`) и ожидают review и слияния; задачи 2 и 9 требуют собственных разрешений. Платные эксперименты, production-записи и deployment согласуются отдельно.
...
**В этой редакции:** задачи 3–8 и 10 реализованы в ветке `feat/judge-proposal-only` (`b05f04d9`, миграции 0128–0133). Локальная регрессия на этой ветке: 448 прошло и 58 subtests в судейских модулях `weblate/trans/tests/test_judge*.py`, 233 в `test_judge_views.py`, 635 в `weblate/api/tests.py`. Оставшиеся падения среды воспроизводятся и на `main`: два `ModuleNotFoundError: weblate_customization` (пакет не установлен на хосте) и `TranslationAPITest::test_autotranslate` (`engines: weblate` недоступен в этой конфигурации). Платные модели не вызывались: все провайдеры застабены через `http_mock`. Production-записей, deployment и восстановления production нет. Следующее разрешение — review и слияние ветки; задачи 2 и 9, платные модели и производственные операции согласуются отдельно.
