...
**Результат проба (2026-09-21).** Прогон через `./deploy/vps.sh ssh` +
`docker exec hcgameloc-weblate-1 weblate shell`, read-only, 60 юнитов
(4 ключа × 15 языков). Все 13 запаркованных таргетов каждого ключа:
`state == 100`, `original_state == 0`, target пустой, `extra_flags ==
'read-only'`. Английский и русский: `state == 20`, флага нет. Аномалий
(`state == 100` и `original_state == 100`) нет. Инвариант «оригинал
сохранён» подтверждён; `update_state` вернёт запаркованные юниты в
`original_state == 0` (STATE_EMPTY).
