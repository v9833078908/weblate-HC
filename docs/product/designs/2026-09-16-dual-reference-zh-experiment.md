...
## Выбор моделей владельцем

Gemini 3.7 через OpenRouter используется для генерации A–D и editor в E/F.
Review в E/F выполняет одна и та же пара действующих production-судей через
LiteLLM, независимо по одному проходу на seat. Точные model IDs, параметры
и правило объединения замечаний фиксируются в preflight по эффективным
конфигурациям; название LiteLLM не является model ID. Модельная финальная
диагностика, если нужна, также идёт через LiteLLM, учитывается отдельно и
не заменяет независимую human LQA. Подробности — версия 1.1 плана исполнения.
Выбор моделей не означает согласования датасета или запуска.
...
Для E и F фиксируются одинаковые два reviewer через LiteLLM и editor Gemini
3.7 через OpenRouter: по одному вызову каждого reviewer и один edit,
одинаковые параметры и максимальные токены. Editor может вернуть текст
...
## Локальные данные для следующей сессии

Пакет подготовлен: `/Users/eli/Downloads/dual-reference-zh-2026-09-16/`;
переносимый архив — `/Users/eli/Downloads/dual-reference-zh-2026-09-16.zip`.
Внутри `pilot-120.jsonl`, полный пул 1113 пар, исходные выгрузки и глоссарии.
Для датасета обращаться к production не требуется. Состав пока предложенный,
human review не подтверждена. Пути, хеши и правила чтения — §5a плана
`docs/product/plans/2026-09-16-dual-reference-zh-study.md`.
...
B и при двух судьях на каждое условие добавляют 4N review + 2N edit операций.
При N=120 это 1200 сегментных операций до повторов; batching может уменьшить
число API-запросов. Отдельная модельная оценка outputs в эту сумму не входит. Двойная
...
Согласованы zh-Hans, Gemini 3.7 через OpenRouter для перевода и LiteLLM
для судей. Датасет остаётся на выборе владельца; экспериментальные результаты
пока отсутствуют.
