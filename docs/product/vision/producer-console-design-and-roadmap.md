...
Пункт 2.6 добавлен отдельно: сначала нативное согласование повторов, затем
адаптер и экран консоли над теми же решениями. Критический путь 1A уже принят
и не ждёт этой вехи.
...
- [ ] **D2-7:** повторы доступны отдельной групповой очередью проекта/языка;
  preview и применение используют нативные решения, approved и независимые
  вхождения защищены. Открытие очереди не запускает LLM; рекомендации
  требуют отдельного подтверждения scope и лимита.

**Доказательство:** API/E2E из 2.1–2.6, файл повторного экспорта, история
Unit, фактическое письмо, сверка расходов и общих решений Weblate/консоли.
...
- Результат: одна строка очереди на Unit с активным блокирующим чеком
  ...
  исключение его из UI: для него есть переход к групповой очереди 2.6,
  отдельно от Unit-счётчика и блокирующих находок.
...
**2.6 Согласование повторов и управляемое reuse** — `todo`, после нативной реализации

- Основание: `docs/product/plans/2026-09-17-repeat-drift-reconciliation-and-managed-reuse.md`.
  План на утверждении; этот пункт не означает, что нативная функция уже готова.
- Результат: очередь групп точного source в проекте и target language,
  фильтры компонентов/меток; варианты, контексты, approved-конфликты,
  независимые вхождения и stale-решения. Считать группы и вхождения раздельно.
- Действия: preview и явное применение одной группы или выбранного пакета,
  общий/независимый вариант, guarded undo. Правила компонентов и source labels
  сохраняются для будущих загрузок. Не создавать отдельную базу canon консоли
  и не менять семантику нативных решений.
- Рекомендации LLM: подготовка всей подтверждённой очереди отдельной командой
  либо явный opt-in при MT; отдельный request cap, unknown cost не равен нулю.
  Результат read-only до подтверждения; вход на экран бесплатен.
- Файлы: существующие `weblate/api/producer/`; проектируемые нативным планом
  `weblate/trans/repeats.py`, `weblate/trans/repeat_recommendations.py`,
  `weblate/trans/models/repeat.py`; будущий экран
  `console/src/screens/RepeatQueue.tsx`. API-адаптер не дублирует правила записи.
- Проверка: один и тот же набор решений виден в Weblate и консоли; поздняя
  правка Advanced даёт conflict, approved не заменяется, разрешённый новый
  повтор использует принятое решение. Browser/API сценарий подтверждает
  отсутствие paid calls при чтении, явный preview пакета и сохранность
  независимых вхождений после повторной загрузки.
