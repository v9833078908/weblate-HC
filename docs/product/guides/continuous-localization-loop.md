...
| LLM-автоперевод: `Could not parse ... JSON` | Модель вернула невалидный JSON; разовый сбой батча, повторить автоперевод безопасно — он идемпотентен (`state:<translated`) |
| LLM-автоперевод: `Mismatching assistant reply ...` | Не разовый сбой модели, а нарушение контракта ответа (потерян/задвоен/перемещён через разметку плейсхолдер и т.п.): смотреть `LLMUsageLog.refusal_reason` и ограниченную выдержку `reply_excerpt` по конкретной строке; повтор автоперевода не лечит повторяющееся нарушение контракта — нужен фикс на стороне промпта/модели или парсера |
