...
2. **Подготовка - фоновая задача, не HTTP-запрос.** Загрузка сохраняет
   `LocKitImportDraft` (`kind=STRING`, `target_component`, owner- и
   session-bound) в состоянии `PREPARING` и публикует
   `prepare_loc_kit_string_update` тем же producer-протоколом, что и остальные
   пользовательские задачи (`INTERACTIVE_TASK_PRIORITY`, детерминированный
   `task_id`, task liveness). Таблица читается через ту же schema v3
...
4. **Confirm запускает фоновую задачу порциями, не одну HTTP-мутацию.**
   `apply_loc_kit_string_update_draft` обрабатывает подготовленный packet
   порциями по `LOC_KIT_STRING_UPDATE_PORTION_SIZE` (25) строк, держа
   `component.locked_for_update()` только на время одной порции - не на всю
   таблицу. Каждая порция вызывает
   `weblate.trans.loc_kit.apply_loc_kit_string_update`, который выполняет обе
   оси как независимые успехи:
   ...
   - Explanation - общий `apply_kit_explanations`; без `source.edit` эта ось
     ...
     снимке preview.
   `append_translation_strings` и `apply_kit_explanations` - две независимые
   транзакции внутри одной порции (каждая со своим локом). Если запись
   строк уже закоммитилась, а Explanation-ось той же порции затем упала
   (например, компонент залочили между двумя вызовами), курсор `next_row`
   не продвигается за эту порцию, и retry (п. 6) её повторяет; повтор для
   уже добавленных в ней ключей корректно попадает в `existing`, а не
   дублирует запись - идемпотентный повтор, не потеря и не дубль строки.
...
6. **Retry - новый `apply_task_id`, тот же курсор.** Упавшая задача
   (`WeblateLockTimeoutError`, `ValidationError`/`ValueError` или иная
   ошибка) переводит черновик в `FAILED` с `retry_phase` (`prepare`/
   `apply`/`finalize`) и сохраняет `next_row`/`progress` как есть; кнопка
   Retry на preview публикует новую задачу с новым `task_id`, не трогая
   курсор и накопленные счётчики - работа продолжается, не начинается
   заново. `action=cancel` до начала применения удаляет черновик без
   изменений в компоненте; после первой успешно применённой порции cancel
   недоступен - остановить может только сама фоновая задача.
7. **Finalizing - один git commit на весь batch.** После последней порции
   `commit_pending_subset` коммитит ровно `PendingUnitChange`, помеченные
   этим черновиком (`metadata["loc_kit_draft_id"]`), не весь
   `commit_pending()` компонента - параллельная ручная правка или другой
   импорт того же компонента не попадают в этот коммит и не блокируются им.
...
11. **Бюджет доставки, ограниченный lock-retry и предел 24 часа.** Один
    ...
    interactive priority - сообщение никогда не переживает свой visibility
    timeout в невидимом `unacked`. `WeblateLockTimeoutError` повторяется
    ограниченно (Celery `countdown` 1…128 с, до 8 попыток), освобождая
    воркер между попытками. Суммарный предел применения - 24 часа от
    первого confirm (`apply_started_at`): он завершает уже записанные
