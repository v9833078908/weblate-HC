...
- Замер глоссарного гейта:
  `docs/misc/2026-08-11-glossary-enforcement-analysis.md` и план
...
вердикт: critical -> reject, major -> flag, иначе pass. Глоссарий и
developer note подаются судье как контекст. Для flag/reject — повторный
прогон 3x с агрегацией перед записью (паттерн GEMBA V2), чтобы сбить
false rejects; pass не переспрашивается.
...
- **Трек A:** эксперимент `response_format: json_schema` на пути
  ...
  Дизайн эксперимента согласуется отдельно и фиксируется планом в
  `docs/plans/`.
- **Трек B:** калибровка судьи на размеченном золотом наборе COL4 (три
  ...
  go/no-go для фазы 2. Тоже отдельный план.
...
от трека B фазы 0 (калибровка) и задач 3-4 фазы 1 (иначе судья получает
ложные hard-команды). Фаза 3 зависит от фазы 2 (канарейка) и внешних
входов плана гейта (доступ к game repo). Фаза 4 — только по данным
фаз 2-3.
