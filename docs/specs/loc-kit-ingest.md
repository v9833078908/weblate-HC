...
2. **Детерминированный вывод (первым, локально, бесплатно).** Из заголовка
   ...
   заполненные в каждой строке-**термина**. Любая заполненная не-языковая
   колонка отклоняется (`InferenceError`) - парсер не угадывает назначение
   колонки. Каждый непрерывный блок строк классифицируется отдельно:
