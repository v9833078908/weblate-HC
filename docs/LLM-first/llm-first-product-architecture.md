...
- **Трек B:** калибровка судьи на размеченном золотом наборе COL4 (три
  ...
  go/no-go для фазы 2. Промпт судьи schema-enforced с первого прогона
  калибровки — тот же контракт, что пойдёт в прод. Тот же план.
...
hard-команды). Фаза 3 зависит от фазы 2 (канарейка) и внешних входов
плана гейта (доступ к game repo). Фаза 4 — только по данным фаз 2-3.
