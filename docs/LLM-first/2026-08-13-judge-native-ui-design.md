...
4. золотой набор пересобран после любой правки, меняющей, какие термины
   попадают в промпт (см. «Известные временные разрывы»);
...
- **Терминологический слой золотого набора не отражает продовый режим
  отказа.** Проверено по `docs/misc/col4-judge-golden.json`: все 195
  записей с `defect_class: terminology` имеют `label_origin: construction`
  и `matcher_bucket: visible` — это ровно один и тот же набор записей.
  То есть каждый терминологический дефект посажен на термин, который
  матчер **и так видит**, тогда как продовый отказ (замер
  `docs/specs/2026-08-11-glossary-enforcement-analysis.md:140-170`)
  происходит там, где термин из промпта **выпал** из-за русского падежа:
  соблюдение 100% при видимом термине против 76.9% при невидимом.
  Записей второго вида в наборе ноль. Судья калибруется на терминологии
  в условиях, противоположных тем, где локализация ломается.
