...
- `docs/LLM-first/2026-08-19-severity-recalibration-final.md` (полный замер n=5, гейты R1-R3)
...
> замером: плечо H2 в драйвере `docs/misc/st2-zh-recalibration.py`
...
4. **`RepeatDriftCheck`**
   (`plans/2026-08-14-intra-component-consistency-check.md`)
...
| **2. Навигация и готовность** | `plans/2026-08-13-02-judge-navigation-and-readiness.md` | `judge_field` в `search.py`, регистрация в `FILTERS`, ключи в выпадашке формы, `calculate_judge()`, карточка «Оценка ИИ» на обзоре, баннер готовности, таблица по языкам, `NOT has:judge` как дефолтный `q` режима `judge`, счётчик строк и оценка цены под фактический `q` формы | на вопрос «можно ли отгружать билд» отвечает один экран; `NOT has:judge` показывает непокрытое; счётчик обещает ровно то, что прогон осудит |
...
- **До плана 2 счётчик строк в форме считает не то, что будет осуждено.**
  ...
  траты денег (замер: `docs/misc/judge-first-dev-run-2026-08-20.md`).
