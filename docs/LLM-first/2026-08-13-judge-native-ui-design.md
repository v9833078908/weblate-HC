...
| B2 — прогон судей | план калибровки | **выполнен 2026-08-13** на train+dev, `col4-judge-eval.py` отгружен; test не тронут | да; **каскад замером не подтвердился**, см. ниже |
...
Строка B2 требует пояснения, потому что она задевает решение этого
документа. Замер 2026-08-13
(`docs/LLM-first/2026-08-13-phase0-measurements.md`) показал, что каскад
проигрывает одиночному судье на всех четырёх измеренных конфигурациях:
сквозной recall 70.7-87.1% против 94.0% у одиночного
`deepseek/deepseek-v4-pro`. Причина не в подборе пары, а в посылке:
ступень A даёт специфичность 97.6-99.2%, то есть ложных тревог, ради
снятия которых заводилась ступень B, почти нет, и её переголосования
снимают настоящие дефекты (46 ошибочных снятий из 48 в худшей паре).
Лестница эскалации из раздела «Лестница эскалации» в текущем виде — с
правом ступени B понижать вердикт — подлежит пересмотру до реализации
плана 1: сквозной recall такой конструкции по построению не может
превысить recall ступени A.
