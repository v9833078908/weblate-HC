...
**Рендер-дефекты плейсхолдеров судье не принадлежат — тем же доводом.**
Юнит 24207 (`ID_AI_PLAYER_RANK_STATUS_NOTIFICATION_DESC`) пропущен всеми
30 прогонами всех плеч рекалибровки: дефект существует только в
отрендеренной строке («3 个国家 盟国 的状态…» — роли слотов читаются
неверно), а судья видит сырую строку с `{0}`/`{1}`, в которой дефекта
нет. Формулировкой промпта это не лечится (R3). Проверяемая гипотеза —
менять **вход**, а не формулировку: детерминированная подстановка
примерных значений в плейсхолдеры (превью в UI юнита; отрендеренные
варианты в промпт судьи). Плечо D замера ставится 2026-08-19; вывод —
в приложении к финальному документу замеров.
