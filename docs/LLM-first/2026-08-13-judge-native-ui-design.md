...
| Backfill автофиксов `--apply` | layer0-план, задача 8 | **не выполнен на проде**, gated-действие | да, предусловие боевого прогона |
...
|---|---|---|---|
| **1. Вердикт** | `plans/2026-08-13-judge-verdict-core.md` | `JudgeVerdict` + миграция, классы judge-чеков, клиент судьи, лестница A→B→починка в Celery, режим `judge` в `AutoForm` + чекбокс перезаписи, fail-safe state 10, гейт по severity, карточка на юните, обратный перевод в форме, протухание, исключение судейских чеков из «Things to check» | прогон на фильтре даёт построчный вердикт; `critical` не уходит в сборку; навигация через `check:judge-*` |
| **2. Навигация и готовность** | `plans/2026-08-13-judge-navigation-and-readiness.md` | `judge_field` в `search.py`, регистрация в `FILTERS`, ключи в выпадашке формы, `calculate_judge()`, карточка «Оценка ИИ» на обзоре, баннер готовности, таблица по языкам | на вопрос «можно ли отгружать билд» отвечает один экран; `NOT has:judge` показывает непокрытое |
| **3. Решения и белый ящик** | `plans/2026-08-13-judge-decisions-and-whitebox.md` | кнопки решений с обязательной причиной, `resolution` в работе, вкладка «Оценка ИИ» с историей лестницы, парсер судейских спанов в `Formatter`, отдельный класс бейджа | продюсер закрывает петлю с аудит-следом; видно, почему судья решил так |
...
2. прод-backfill автофиксов `--apply` выполнен по runbook layer0-плана;
3. отпечаток автофиксов золотого набора совпадает с отпечатком
   прод-инстанса (иначе судья калиброван на дефектах, которые слой 0 уже
   снимает);
4. отдельное согласование траты и изменения прод-данных.
