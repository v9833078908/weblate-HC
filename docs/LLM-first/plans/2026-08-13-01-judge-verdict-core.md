...
> Системный промпт клиента (задача 5) берётся из **плеча C** замера:
> обязательный `description` + рубрика severity «последствие для
> игрока» (`docs/misc/st2-zh-recalibration.py`, `RUBRIC_RULE`) — лучший
> измеренный профиль (шум −23%, REAL@14 12/14, precision 0.56).
> Регулятор «второй судья только для повышения до critical» из дизайна
