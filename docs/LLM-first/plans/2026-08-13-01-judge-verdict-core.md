...
`JudgeVerdict`; `critical` не уходит в сборку без решения человека;
судейские строки находятся через `check:judge-flag` / `check:judge-reject`.

> **Поправка 2026-08-19** (полный замер
> `docs/LLM-first/2026-08-19-severity-recalibration-final.md`, гейты
> R1-R3): severity судьи — не гарантия (пропуски настоящих critical
> 1-5 за прогон, ложные critical 4-5/124), поэтому `critical → state 10`
> реализуется как в плане, но читается как **очередь на решение
> человека**, а не автоматический гарант «сломанное не уйдёт».
> Конфигурация клиента (задача 5) — **плечо D** замера: промпт плеча C
> (обязательный `description` + рубрика severity «последствие для
> игрока», `RUBRIC_RULE`) **плюс** детерминированные рендер-превью
> плейсхолдеров во входе (`render_preview`, `RENDER_RULE` в
> `docs/misc/st2-zh-recalibration.py`). Плечо C — лучший профиль по шуму
> и precision; рендер-превью поверх него поднимают самый опасный класс
> (плейсхолдер-critical: 24207 0/5→4/5, 24130 2/5→5/5, ячейка
> `critical→none` 4→0) при плоском шуме — см. приложение к финальному
> замеру. Регулятор «второй судья только для повышения до critical» из
> дизайна замером отвергнут — не реализовывать.
...
> **Поправка 2026-08-19.** Новый срез снят, эффект измерен: сам по себе
> обязательный `description` метрики не улучшает (R1: precision
> 0.51→0.54 при recall 0.75→0.67), его ценность — улики. Метрики
> возвращает рубрика severity поверх него (плечо C). Оба текста правил —
> `DESCRIPTION_RULE` и `RUBRIC_RULE` в `docs/misc/st2-zh-recalibration.py`.
...
1. Во вью юнита положить в контекст `judge_round` — `latest_round(unit)`,
   ...
   перечисляет мнения обоих мест: согласие мест — сильный
