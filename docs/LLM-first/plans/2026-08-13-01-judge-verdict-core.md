...
**Основание:** `docs/LLM-first/2026-08-13-judge-native-ui-design.md` (разделы 1-4,
...
> `docs/LLM-first/2026-08-19-severity-recalibration-final.md`, гейты R1-R3):
...
> `RENDER_RULE` в `docs/misc/st2-zh-recalibration.py`). Плечо C — лучший профиль по шуму
...
|---|---|
| План 2 | `judge_field` в `search.py`, регистрация в `FILTERS`, судейские ключи в выпадашке формы, `calculate_judge()`, карточка «Оценка ИИ» на обзоре языка, баннер релизной готовности, `NOT has:judge` как дефолтный `q` |
...
1. **Дефолтный `q` режима `judge` остаётся `state:<translated`.** Дизайн хочет
...
формы измеренного драйвера (`docs/misc/st2-zh-recalibration.py`: `build_payload`,
...
из `docs/misc/st2-zh-recalibration.py:72-165`, скопировать дословно). Языковую пару и
...
4. Записать наблюдение (сколько строк, на скольких места разошлись, сколько починок,
   сколько `unparsed`) в `docs/misc/` отдельным файлом.
