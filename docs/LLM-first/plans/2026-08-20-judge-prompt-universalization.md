...
Дата: 2026-08-20. Статус: **написан, ждёт одобрения.**
...
Правило R3 дизайна (`docs/LLM-first/2026-08-13-judge-native-ui-design.md:241`):
...
| Зашитый в промпт жанр WWII дал ложный `major` на CoL4 | `docs/misc/judge-first-dev-run-2026-08-20.md`, находка 5 |

Внешние источники по промпту судьи собраны отдельно:
`docs/LLM-first/2026-08-20-judge-prompt-best-practices.md` (PoLL arXiv 2404.18796,
...
6. Дымовой прогон на dev по существующему сценарию
   `docs/misc/2026-08-20-judge-dev-test-scenario-col4.md`.
...
`docs/misc/st2-zh-recalibration.py` - самостоятельный офлайн-скрипт: корпус,
...
```bash
python3 docs/misc/st2-zh-recalibration.py --arm E --dry-run
python3 docs/misc/st2-zh-recalibration.py --arm F --dry-run
python3 docs/misc/st2-zh-recalibration.py --arm G --dry-run
```
...
```bash
git add docs/misc/st2-zh-recalibration.py
...
OPENROUTER_API_KEY=... python3 docs/misc/st2-zh-recalibration.py \
    --arm E --model deepseek/deepseek-v4-pro --repeats 5 --out-dir docs/misc/st2-zh-recal
OPENROUTER_API_KEY=... python3 docs/misc/st2-zh-recalibration.py \
    --arm E --model qwen/qwen3-235b-a22b-2507 --repeats 5 --out-dir docs/misc/st2-zh-recal
```
...
```bash
python3 docs/misc/st2-zh-score.py --truth docs/misc/st2-zh-groundtruth.json \
    --out-dir docs/misc/st2-zh-recal
```

### Step 3: Отчёт

`docs/misc/2026-08-20-judge-prompt-universalization-run.md`: таблица D / E / F / G
...
По существующему сценарию `docs/misc/2026-08-20-judge-dev-test-scenario-col4.md`
...
- [ ] Плечи E, F, G воспроизводимы из закоммиченного драйвера, `--dry-run`
      показывает ожидаемый промпт и схему.
- [ ] Прогон E и F: 2 модели x 5 повторов, файлы в `docs/misc/st2-zh-recal/`.
- [ ] Отчёт содержит все пять метрик по каждому плечу и явный вердикт по
      каждому гейту D6.
- [ ] `missed_crit` = 0 в каждом прогоне плеча, которое едет в код.
- [ ] Промпт в коде содержит `{project_context}` и не содержит ни одного
      названия жанра.
- [ ] Дефолт без настройки проекта - нейтральный фолбэк, зафиксировано тестом.
- [ ] `{name}` даёт рендер-превью; мутация регулярки роняет тест.
- [ ] Без настройки `JUDGE_REASONING_EFFORT` тело запроса не содержит
      `reasoning`.
- [ ] Дымовой прогон на dev: «Сэр» больше не `major` по регистру.
- [ ] Судья на проде остался выключенным.
