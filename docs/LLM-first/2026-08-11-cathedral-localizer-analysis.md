...
structured output. Наша актуальная связка фазы 0
(`docs/LLM-first/plans/2026-08-11-phase0-schema-and-judge-calibration.md`)
и архитектуры LLM-first (часть 4) закладывает строгую JSON-схему, судью
из другого семейства моделей, чем переводчик, и калибровку на золотом
наборе. Cathedral даёт полевое подтверждение, что все три решения
обязательны, а не факультативны.
