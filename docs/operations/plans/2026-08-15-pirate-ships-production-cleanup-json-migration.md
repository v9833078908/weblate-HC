...
Отдельно: триггер `weblate-push-translate` «обнови перевод <id>
<new_value>» передаёт значение как русский исходник. По журналу Weblate
так RU получил английский текст у `daily39_description`,
`cbp_use_amulet_battle`, `cbp_use_amulet_battle2` и (2026-09-24)
`tw_item_name_building_portal`. Правка перевода на конкретном языке должна
менять target этого языка, а не RU.
...
| 2 | Свести Weblate с beta: 277 переименований, 158 новых ключей, 8 удалений, 595 ячеек по правилам черновика `analysis/data/pirate-ships-reconciliation-2026-09-24/` (скрипт `analysis/probes/pirate-ships-reconcile.py`, пересчитать после заморозки); спорный RU подтверждает команда | Мы, запись в production по отдельному разрешению | Повторная сверка: расхождения только из согласованного списка |
