...
### 3. Пересчёт проверок не делаем

Решение владельца от 2026-08-27: проверки должны работать для **следующих**
переводов; пересчитывать то, что уже лежит в БД, не требуется.

Что это значит технически. Строки проверок хранятся в `checks_check` и
перезаписываются только внутри `Unit.run_checks()` (`unit.py:2238-2243`),
который вызывается при сохранении или импорте юнита. Поэтому:

- любая строка, которую переводчик или автоперевод тронет после деплоя,
  сразу получает новый набор проверок - включая условный `max-length` и новый
  `game-length`;
- строки, которых никто не касается, сохраняют прежние сработки, пока их не
  подберёт штатный `daily_update_checks` (`weblate/trans/tasks.py:1206-1220`);
  при `BACKGROUND_TASKS = "monthly"` (дефолт, в проде переменная не задана) это
  примерно раз в месяц на компонент.

Это принятое состояние, а не дефект: цифры в отчётах по старым строкам какое-то
время отражают прежние правила. Массовый `weblate updatechecks` остаётся
доступным вариантом на будущее, если понадобится единая картина - тогда его
стоит планировать отдельно, потому что `GameLengthCheck` включён по умолчанию
(`checks.py:826`, `default_disabled = False`) и на прод-данных дал бы порядка
тысячи новых сработок примерно на 172 тыс. переведённых юнитов.
...
- **Простой.** Пересборка образа плюс рестарт; здоровье проверяет сам скрипт по
  ...
  переводчиками.
- **Снятие штатных проверок длины** оставляет прежние сработки `max-length` на
  старых строках до их следующего сохранения (пункт 3). Данные строк не
  затрагиваются.
...
5. В браузере на странице прод-проекта видна полоса `AI judge`, обе ссылки
   отдают 200.
6. Поведение подтверждено на условной строке: при бюджете 40 проверка
...
7. Новый перевод в интерфейсе: сохранить строку в любом компоненте и убедиться,
   что набор проверок на ней пересчитался. Это и есть требуемое поведение -
   проверки работают для следующих переводов.
