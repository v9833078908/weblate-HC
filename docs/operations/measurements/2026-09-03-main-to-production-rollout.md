...
свежих judge-строк остались `cost_usd IS NULL`. `priced_complete=yes`
недостижим без отдельного решения; варианты - выключить стриминг у судей,
считать цену локально по `/model/info`, или дотягивать её из
`/spend/logs?request_id=<x-litellm-call-id>` (оба эндпоинта отвечают 200 с
судейским ключом). Машинный перевод не затронут: он идёт через OpenRouter,
который отдаёт `usage.cost`.
...
- **Цена judge-вызовов** (фаза 3): решение владельца, см. таблицу выше.
