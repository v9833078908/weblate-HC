...
## Фаза 8. Backfill кандидатов (выполнена 2026-09-04)

Бэклог активных нерешённых `judge-reject`/`judge-flag` - 216 юнитов в пяти
парах компонент-язык (`heart-abyss/temple/en` 142, `strategy-and-tactics-2/glosssary/ru`
33, `victory-banner/common/fr` 20 и `/es` 14, `col4/data/fr` 7), но платного
вызова требуют лишь те, у которых коллегиальный вердикт всё ещё
`reject`/`flag`, не решён, нет активного кандидата и нет активного
`max-length`. Dry-run по каждому проекту (аргумент - слаг проекта; `slug/*`
команда не понимает):

| Проект | `pending` | Записано |
|---|---|---|
| `col4` | 7 | 7 `generated` |
| `heart-abyss` | 4 | 4 `generated` |
| `strategy-and-tactics-2` | 0 | - |
| `victory-banner` | 0 | - |

Запуск: `weblate judge_backfill_candidates <project> --write --user
i.efimov@herocraft.com --limit <pending>`. 11 платных ремонтных вызовов,
все через `OpenRouter` (у обоих проектов настроен свой routed-движок),
ошибок и `drift` нет; 38 s на `col4`, 22 s на `heart-abyss`. Повторный
dry-run после записи: `0 pending` в обоих проектах, все 11 юнитов
классифицируются как `existing`.
...
`Перепроверить`. После фазы 8 та же карточка отдаёт и строку кандидата
(`data-judge-candidate="1"`) с кнопкой `Принять исправление` - то же на
юните 22578. Проверка до backfill искала несуществующий маркер
`Использовать`: шаблон печатает `Use suggested fix`, что в ru-локали
`Принять исправление`.
